...
Affix Przeistoczenia nie wymaga od użytkownika wpisywania source rolla `0/25`. Formularz używa pola `Wartość widoczna na itemie`, bo użytkownik zwykle przepisuje wartość z gry po Doskonaleniu. UI opisuje pochodzenie wartości jako `Wartość widoczna w grze`, `Wartość bazowa/source roll` albo `Nieznane pochodzenie`. Model `TransfigurationAffixRoll` zapisuje `displayedValue` i `valueProvenance`:
...
- linia przeistoczenia `+115 pkt. do wszystkich współczynników [75 - 100]` trafia do `ItemTransfiguration` jako `ALL_STATS` z wartością prezentowaną `115`; pozostaje to warstwą przeistoczenia, a nie losowym ordinary affixem, i nie aktywuje nowego runtime bez osobnego kontraktu,
