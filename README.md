...
- import unikatowej broni kataloguje i prezentuje zwykłe affixy `Obrażenia od broni`, `Zdrowie za zabicie` oraz `Mnożnik obrażeń z upływem czasu`; te affixy zachowują wartość i zakres rolla z tooltipa, ale pozostają `Runtime nieaktywny`, jeżeli nie mają osobnej bezpiecznej ścieżki runtime,
...
- GUI pokazuje sekcję `Pełny odczyt widocznego itemu` jako krótki podgląd OCR/metadanych itemu z nagłówkiem, sekcją `Linie bazowe`, dodatkowymi liniami i socketem / gniazdem; affixy nie są renderowane w tej sekcji i są edytowane wyłącznie w tabeli ręcznej weryfikacji affixów, a aspekt / efekt jest widoczny tylko w bloku `Aspekt / efekt` formularza potwierdzenia,
- `Pełny odczyt widocznego itemu` nie dubluje pól edycyjnych specyficznych dla typu itemu: dla broni pola `DPS broni`, min/max/średnia obrażeń za trafienie i `Ataki na sekundę` są edytowane wyłącznie w fieldsecie `Dane broni`, a dla tarczy `Pancerz` jest edytowany wyłącznie w fieldsecie `Dane tarczy`,
...
- GUI po zatwierdzeniu pokazuje `Pełny odczyt zapisany w bibliotece` jako podgląd OCR/metadanych itemu i również nie dubluje pól edycyjnych typu itemu,
