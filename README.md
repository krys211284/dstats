...
- kolumna `Item` pokazuje nazwę itemu, rzadkość i moc przedmiotu, jeśli są dostępne; item założony przez aktywnego bohatera dostaje subtelny badge `Założony` przy nazwie zamiast osobnej kolumny statusu,
- kolumna `Slot / typ` pokazuje slot ekwipunku i typ itemu,
- kolumna `Aspekt` pokazuje finalny aspekt z `selectedAspectId` przez nazwę z registry albo `Brak`,
- kolumna `Affixy` pokazuje krótki skrót zatwierdzonych affixów; `Greater Affix` jest prezentowany bezpośrednio przy konkretnym affixie jako gwiazdka, np. `★ 13,2% redukcji czasu odnowienia`, a nie jako osobny status itemu,
- akcja `Szczegóły` rozwija pełny semantyczny widok itemu jako osobny detal, a nie stałą ciężką kartę w głównej liście,
- szczegóły itemu pokazują osobne sekcje `Dane podstawowe`, `Base stats`, `Implicit / linie bazowe`, `Affixy`, `Aspekt / efekt legendarny`, `Socket / gniazdo`, źródło itemu oraz diagnostykę OCR tylko wtedy, gdy istnieją faktyczne linie diagnostyczne; pusta sekcja `Diagnostyka OCR` nie jest renderowana,
...
- akcja `Załóż bohaterowi` pojawia się dla zgodnego pustego slotu aktywnego bohatera, a `Zmień w slocie` dla zgodnego slotu, który ma już inny item,
...
- akcja `Szczegóły` rozwija pełne semantyczne sekcje itemu bez wracania do ciężkich kart jako głównej listy,
...
- GUI biblioteki pokazuje rozwijane `Szczegóły`, jeżeli item ma zapisane linie `FullItemRead`,
...
- GUI biblioteki pokazuje akcję `Załóż bohaterowi` dla zgodnego pustego slotu oraz `Zmień w slocie` dla zgodnego slotu, który ma już item aktywnego bohatera,
- GUI biblioteki pokazuje akcję `Pokaż slot w current build`, która prowadzi do ekranu aktualnego buildu po zmianie albo przeglądzie itemu,
...
- rozwijane `Szczegóły` itemu z pełnymi sekcjami semantycznymi oraz prezentacyjną gwiazdką `Greater Affix` bezpośrednio przy affixie w skrócie affixów,
