...
Import itemu z wielu screenów jest przeznaczony dla przewijanego tooltipa, który nie mieści się na jednym ekranie. Formularz pozwala wybrać `1..5` obrazów tego samego itemu; screeny trzeba dodać w kolejności od góry tooltipa do dołu. Importer wykonuje OCR każdego obrazu osobno, scala komplementarne warianty OCR z danego screena, potem scala screeny w kolejności uploadu, usuwa overlap oraz linie UI-only typu `Przewiń w dół`, `Przewiń do góry`, akcje ekwipunku, sprzedaż, trwałość i porównawcze `(Wytrzymałość: +4,6%)`. Parser dostaje wyłącznie kanoniczny tekst źródłowy, czyli jeden logiczny opis itemu; surowe multi-OCR variants i canonical source nie są pokazywane w normalnym UI importu. Zduplikowane bazowe/implicit linie z overlapu albo wariantów OCR, np. `20,0% szansy na blok [20,010]%` i `+100% obrażeń od broni w głównej ręce [100]%`, są deduplikowane do kanonicznych linii bazowych; `+100% obrażeń od broni w głównej ręce` jest base line, a nie zwykłym affixem. Aspekt `Gdy masz umocnienie... 61%[x]` musi przetrwać canonical merge także przy uszkodzonych wariantach OCR typu `o61`, `610[x]` i `651%`.
