...
- GUI w tej sekcji rozdziela aktualne dane przypisanej umiejętności od konfiguracji runtime; dla `SkillId.CLASH` prezentuje nazwę `Starcie`, aktualną rangę, kategorie, jawny procent obrażeń dla tej rangi, Lucky Hit, bazowe generowanie Wiary oraz docelowy model wyborów grupowych Starcia. `Marsz Krzyżowca` jest pokazywany jako stały bazowy efekt przypisanego Starcia, a nie wybór użytkownika; stare zapisy z wyłączonym `baseUpgrade_CLASH` są normalizowane tak, żeby UI nie prezentowało Starcia jako pozbawionego efektu bazowego. Wybory użytkownika dotyczą po jednej opcji z `choiceGroup1_CLASH`, `choiceGroup2_CLASH` i `choiceGroup3_CLASH`. Grupa 1 zawiera `Generowanie Wiary` i `Animusz`, grupa 2 zawiera `Skuteczność Marszu Krzyżowca` i `Zwiększenie Obrażeń`, grupa 3 zawiera `Brać Ich`, `Potyczka` i `Kara`; lista pochodzi z `PaladinSkillTreeRegistry`. Sekcja `Ulepszenia Starcia` rozdziela rząd pól sterujących od rzędu opisów wybranych efektów; to zmiana prezentacyjna, bez zmian runtime DPS i golden values. Efekty oznaczone jako aktywne w runtime wpływają na symulację; pozostałe modyfikatory są obecnie opisowe.
...
- Golden runtime dla `Moloch + Starcie + choiceGroup1_CLASH=animusz + initialAnimus=1`, `10 s`, `100/100` Wiary i Verathiela: kroki `1..4` są normalne i zmieniają Animusz `1 -> 3 -> 5 -> 7 -> 8`; krok `5` aktywuje Molocha przed hitem, zużywa `8`, trafia za `3380`, a po trafieniu wraca do `3`; krok `9` ponownie aktywuje Molocha. Wynik: `2` aktywacje Molocha, `6` buffowanych hitów, total `28732`, UI `DPS = 2873,2`, końcowa Wiara `65`, końcowy Animusz `5`, łączny przyrost Animuszu ze Starcia `18`.
- Golden runtime dla `Moloch + Starcie + choiceGroup1_CLASH=animusz + initialAnimus=8`, `10 s`, `100/100` Wiary i Verathiela: aktywacje Molocha wypadają na krokach `1`, `5` i `9`, wszystkie `10` hitów są buffowane, total `33800`, UI `DPS = 3380`, końcowa Wiara `65`, końcowy Animusz `5`, łączny przyrost Animuszu ze Starcia `18`.
