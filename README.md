...
- `Greater Affix` zachowuje zakres rolla, jeżeli OCR albo katalog dostarczył stabilne `rollRangeMin` / `rollRangeMax`; tylko rzeczywisty brak zakresu jest opisywany w UI jako `Bez zakresu (Greater Affix)`, nie jako błąd katalogu,
...
- dla affixu `Redukcja obrażeń` merger preferuje pełny zakres procentowy z wariantu OCR, np. `11,0 - 15,0`, nad uszkodzoną sklejką typu `11 - 0`; jeżeli OCR zachował uszkodzony fragment zakresu i wartość mieści się w katalogowym zakresie definicji, formularz przenosi zakres `11,0 - 15,0` zamiast pokazywać `Brak zakresu`; wartość i zakres pozostają danymi importu itemu i nie są aktywne w runtime DPS,
...
- `Ślad kroków symulacji` scala szczegóły Wiary i Animuszu w dwie zbiorcze kolumny prezentacyjne. Wiara pokazuje w jednej komórce `przed`, `koszt`, `gen.`, `regen.` i `po`, a Animusz pokazuje `przed`, `koszt`, `minimum` przy aktywacji Molocha, `gen.` i `po`; kolumna `Narastająco` nie jest renderowana w tabeli UI, żeby odchudzić debug. `Stan paska` pozostaje w DOM jako zwijany szczegół. Trace pokazuje minimum, żeby wyjaśnić clamp `max(animuszPrzed - 8, 1)`. To jest zmiana prezentacyjna; runtime i golden values pozostają bez zmian.
...
- zachowanie zakresu `+114 siły [107 - 121]`, brak sklejania `+7,0% szansy na szczęśliwy traf [7,0` z `13,2% redukcji czasu odnowienia` oraz ostrożne oznaczanie `Greater Affix`: tylko jawne markery OCR, np. `*`, `★`, `⭐`, `✦`, `•` albo podobny glif gwiazdki/punktu na początku linii, ustawiają `greaterAffix=true`, a sam brak bracketowego zakresu rolla nie wystarcza do automatycznej gwiazdki,
