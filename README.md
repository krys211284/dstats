...
- logi techniczne obejmują sekcje `IMPORT_REQUEST`, `FILE`, `OCR_RAW_VARIANTS`, `OCR_NORMALIZED_LINES`, `OCR_SEGMENTER_INPUT`, `OCR_SEGMENTER_MATCH`, `OCR_SEGMENTER_OUTPUT`, `SCREEN_MERGER_INPUT`, `SCREEN_MERGER_OUTPUT`, `MERGE_DECISION`, `MERGE_REJECTED`, `MERGE_NUMERIC_TOKENS`, `ITEM_DETAILS`, `WEAPON_DAMAGE_RANGE_MERGE`, `AFFIX_CANDIDATE`, `AFFIX_REJECTED`, `AFFIX_MERGE`, `SOCKET_GEM_RUNE_CANDIDATE`, `SOCKET_REGION_LINE`, `TRANSFIGURATION_CANDIDATE`, `ASPECT_MATCH`, `FINAL_IMPORT_FORM`, `FORM_SUBMIT_MAPPING`, `ITEM_LIBRARY_SAVE_READ` i `RUNTIME_CONTRIBUTION`,
...
- `GA_MARKER_DETECTION`, `GA_MARKER_SUMMARY` i `GA_MARKER_FINAL_STATE` zapisują lokalne przypisanie markera GA do kandydata affixu, liczbę markerów globalnych, liczbę markerów lokalnie przypisanych, propagację z `visualAnchorCandidate` do finalnego affixu, finalny `greaterAffix` oraz `ambiguousGaMarkersRequiresConfirmation`; przy markerze globalnym bez bezpiecznego przypisania UI ma dostać `REQUIRES_CONFIRMATION / ambiguous_ga_markers` oraz per-affix stan potwierdzenia,
