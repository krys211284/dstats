...
- sekcja aspektu w imporcie pokazuje finalny wybór `selectedAspectId` jako nazwę aspektu, typ aspektu w etykiecie (`Aspekt unikatowy` dla `UNIQUE`), status runtime, opis aspektu z `AspectRegistry`, osobną sugestię OCR z confidence oraz pomocniczy `Odczyt OCR efektu`, jeśli jest dostępny i sensowny,
...
- GUI pokazuje wybór aspektu jako listę czystych nazw znanych aspektów zgodnych ze slotem itemu; poniżej pokazuje finalnie wybrany aspekt, etykietę `Aspekt unikatowy` dla typu `UNIQUE`, status runtime, opis efektu aspektu z `AspectRegistry`, sugestię OCR jako podpowiedź oraz confidence jako osobny opis,
