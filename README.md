...
Skill może mieć więcej niż jedną kategorię źródłową. Potwierdzone przykłady z lokalnej paczki Basic: `Wymach` ma `Podstawowe, Adept`, `Święty Pocisk` ma `Podstawowe, Sędzia`, `Starcie` ma `Podstawowe, Moloch`, a `Natarcie` ma `Podstawowe, Mobilność, Fanatyk`. Kategorie nadawane dopiero przez ulepszenia, np. wariant „staje się umiejętnością Adepta”, nie są bazowymi kategoriami skilla i pozostają informacją ulepszenia.
...
Kolumny `Koszt Wiary`, `Generowanie Wiary` i `Lucky Hit` są rozdzielone. `Koszt Wiary` pokazuje potwierdzony koszt użycia, np. `Błogosławiony Młot` ma `10`; jeżeli lokalne źródła nie podają kosztu, komórka pokazuje `-`. `Generowanie Wiary` pokazuje bazową generację i potwierdzony dodatek z ulepszenia, np. `Wymach` ma `14; +5 - Generowanie Wiary`, `Święty Pocisk` ma `16; +7 - Generowanie Wiary`, `Starcie` ma `20; +10 - Generowanie Wiary`, a `Natarcie` ma `18`. Jeżeli dodatek pochodzi z ulepszenia, widoczna wartość dostaje krótki tooltip użytkowy z nazwą modyfikatora i treścią, np. `Generowanie Wiary - dodaje +10 Wiary`. `Lucky Hit` jest opisową wartością źródłową z lokalnych materiałów, np. Basic pokazuje `Wymach = 26%`, `Święty Pocisk = 57%`, `Starcie = 63%`, `Natarcie = 18%`; brak danych renderuje `-`, nigdy `0%`. Te wartości są danymi opisowymi UI i sortowania/filtrowania, nie są używane przez `DamageEngine`.

Kolumny faceted renderują skrócone, opisowe dane z `UpgradeDamageModifier`, jeżeli `safeForRankingDisplay = YES`, albo wpisy wymagające ręcznej weryfikacji w `Manual review`. Każda widoczna pozycja modyfikatora ma krótki tooltip użytkowy z prefiksem źródła cechy: `Umiejętność:`, `Modyfikator:` albo `Manual review:`. Tooltipy zawierają szczegóły wartości, ale nie są kontraktem runtime DPS i nie odblokowują mechanik w `DamageEngine`. `Dmg multiplier` oznacza mnożnik obrażeń, np. `20%[X]`; `Dmg bonus` oznacza addytywny bonus obrażeń, np. `20%[+]`; `Extra hit / component` oznacza osobne źródło obrażeń, dodatkowy hit, powrót pocisku albo dodatkowy łuk; `Damage over time` jest zarezerwowane dla DOT/tick/periodic damage. `Status / debuff` pokazuje tylko konkretną nazwę albo wartość statusu/debuffu, np. `20% - Odsłonięcie`, `Spowolnienie`, `Osąd` albo `Osłabienie`; nie renderuje fraz typu `status / pośredni wpływ` ani `pośredni wpływ`. `Defense / utility` i `Manual review` nie zaczynają głównego tekstu od wartości typu `15%[X]`, `25%[X]`, `30%[+]` albo `20%[X]`, jeżeli mogłoby to wyglądać jak zwykły damage multiplier skilla; takie wartości trafiają do tooltipów. Brak danych w komórkach jest renderowany jako `-`. `Speed / cooldown` nie jest już kolumną głównej tabeli, chociaż dane tego typu mogą pozostać w modelu audytowym.

Wartości liczbowe w komórkach modyfikatorów są renderowane na początku tylko tam, gdzie nie zaciemnia to semantyki wpisu, np. `20%[X] — Zwiększenie Obrażeń`, `128% — Miecz Mistrzostwa`, `120% — Krzyżowe Uderzenie, 2 dodatkowe łuki`. Komponent `Potyczka` pokazuje `155% R1`, żeby było jasne, że wartość dotyczy rangi 1 i nie jest sumowana z bazowymi obrażeniami. Puste kategorie pokazują neutralny placeholder `-` z opisem w `title`/`aria-label`; tekst `brak wpływu na obrażenia` nie jest pokazywany w głównej tabeli. Listy komponentów i modyfikatorów są prezentacyjne: nie są sumowane, nie tworzą wartości `razem`/`total`, nie zmieniają prostych tabel `baseDamagePercentRanks`, nie wpływają na runtime DPS i nie odblokowują `DamageEngine`.

Starcie ma w rankingu opisowo pokazane potwierdzone dane z lokalnego Markdown Basic bez liczenia DPS: `Generowanie Wiary` pokazuje `20; +10 - Generowanie Wiary`; `Defense / utility` pokazuje bazowy `Marsz Krzyżowca` jako opis szansy na blok oraz opisowe efekty `Animusz` i `Skuteczność Marszu Krzyżowca`, a wartości szczegółowe są w tooltipach; `Dmg multiplier` pokazuje `20%[X] - Zwiększenie Obrażeń` oraz warunkowy wpis `Brać Ich` bez renderowania `8%[X]` jako bezwarunkowego mnożnika w głównym tekście; `Extra hit / component` pokazuje `155% R1 - Potyczka`; `Manual review` pokazuje `Kara - Odwet / ciernie` bez renderowania wartości cierni jako zwykłego damage skilla. Te wpisy nie zmieniają `baseDamagePercentAtRank1 = 115`, `baseDamagePercentAtTreeMaxRank = 293`, nie są sumowane ze sobą ani z bazą i nie są konsumowane przez runtime.
...
- bieżący `AspectRegistry` jest zalążkowym katalogiem znanych aspektów potrzebnym do stabilizacji importu; nie udaje pełnej produkcyjnej bazy wszystkich aspektów Diablo 4 i zawiera wyłącznie świadomie dodane definicje, obecnie `inner-calm` / `Aspekt Wewnętrznego Spokoju` dla zgodnych slotów ręki dodatkowej,
...
- sekcja aspektu w imporcie pokazuje finalny wybór `selectedAspectId` jako nazwę aspektu, opis aspektu z `AspectRegistry`, osobną sugestię OCR z confidence oraz pomocniczy `Odczyt OCR efektu`, jeśli jest dostępny i sensowny,
...
- OCR confidence aspektu jest osobnym opisem sugestii OCR i nie jest częścią nazwy opcji aspektu w selectcie,
...
- GUI pokazuje ręczny formularz zatwierdzenia z edytowalną listą affixów: użytkownik może zmienić typ i wartość affixu, oznaczyć `Greater Affix`, usunąć wiersz z tabeli albo dodać brakujący affix z katalogu przez akcję `Dodaj affix` bez przeładowania strony,
- GUI pokazuje wybór aspektu jako listę czystych nazw znanych aspektów zgodnych ze slotem itemu; poniżej pokazuje finalnie wybrany aspekt, opis efektu aspektu z `AspectRegistry`, sugestię OCR jako podpowiedź oraz confidence jako osobny opis,
- GUI nie kopiuje raw OCR effect do registry: opis aspektu jest ogólny i nie zawiera zgadywanych procentów rolla, a `Odczyt OCR efektu` jest pomocniczym odczytem konkretnego itemu,
- jeśli OCR wykrył tekst aspektu, ale `AspectRegistry` nie zna dopasowania, GUI pokazuje komunikat o braku dopasowania w katalogu i zostawia finalny wybór aspektu pusty,
...
- zachowanie `greaterAffix=true` przez formularz, zapis biblioteki i ponowny odczyt, rozpoznanie markerów OCR `*`, `★`, `⭐` i `✦` oraz heurystykę rozpoznanego affixu bez bracketowego zakresu rolla,
...
- pokazanie OCR confidence aspektu jako osobnego opisu sugestii, a nie części nazwy aspektu w selectcie,
