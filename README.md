...
- UI importu i edycji pokazuje wykryte staty socket/gem/rune w sekcji `Gniazda`, ustawia `Liczba gniazd` na `totalSocketCount`, pokazuje teksty statów przy odpowiednich gniazdach i oznacza je jako `Runtime nieaktywny`. Mechanizm nie jest wyjątkiem dla `STRENGTH` ani dla `Dziedzica Zatracenia`; różne staty z regionu socket/gem/rune, np. inteligencja, zręczność, siła woli albo pancerz, są zachowywane jako socket data i nie trafiają do ordinary affixów,
...
- normalny ekran importu itemu nie renderuje technicznego `Pełnego zapisu itemu` ani raw dumpu `Socket / gniazdo`; `FullItemRead` pozostaje w modelu, hidden fieldach, submit/roundtrip i repozytorium, ale użytkownik widzi dane w uporządkowanych sekcjach domenowych: `Ręczne potwierdzenie itemu`, `Aspekt / efekt`, `Ręczna weryfikacja affixów`, `Hartowanie`, `Doskonalenie`, `Przeistoczenie / Kostka Horadrimów` i `Gniazda`,
...
UI importu i edycji pokazuje sekcję `Gniazda` z wyborem `Liczba gniazd` (`0`, `1`, `2`) oraz wierszami `Gniazdo 1` i `Gniazdo 2`. Po wybraniu gema formularz pokazuje jego efekt dla kontekstu itemu; dla tarczy `Wspaniały Diament` pokazuje `+30 pkt. do wszystkich współczynników`. Biblioteka i current build prezentują sockety jako osobne linie, np. `Gniazda · Liczba gniazd: 1 · 1: Wspaniały Diament · +30 pkt. do wszystkich współczynników · Runtime nieaktywny`. Import OCR może rozpoznać dokładną linię `Puste gniazdo` jako jedno puste gniazdo oraz wykryte staty zajętych gniazd, np. `+150 siły` i `+120 siły`, jako dwa `DETECTED_STAT`. Takie staty są widoczne w UI i przechodzą przez import form, hidden fields, submit, repository, edit form i ponowny zapis, ale nie aktywują runtime gemów/run.

Normalny ekran importu pokazuje socket/gem/rune stats wyłącznie w sekcji `Gniazda`, np. `Liczba gniazd: 2`, `Gniazdo 1: +150 siły`, `Gniazdo 2: +120 siły` i `Runtime nieaktywny`. Te same linie nie mogą pojawić się drugi raz jako techniczny raw dump `Pełny zapis itemu` / `Socket / gniazdo`. Jeżeli trzeba diagnozować pełny odczyt, source raw line, typed merge metadata albo decyzje parsera, źródłem są debug logi włączane przez `-Ddstats.itemImport.debug=true`, domyślnie zapisywane do `logs/item-import-debug.log`; normalny UI nie dodaje panelu debug.
...
- katalog `logs/` jest lokalnym katalogiem diagnostycznym ignorowanym przez Git; plików takich jak `logs/item-import-debug.log` nie należy commitować,
...
- logi techniczne obejmują sekcje `IMPORT_REQUEST`, `FILE`, `OCR_RAW_VARIANTS`, `OCR_NORMALIZED_LINES`, `OCR_SEGMENTER_INPUT`, `OCR_SEGMENTER_MATCH`, `OCR_SEGMENTER_OUTPUT`, `SCREEN_MERGER_INPUT`, `SCREEN_MERGER_OUTPUT`, `MERGE_DECISION`, `MERGE_REJECTED`, `MERGE_NUMERIC_TOKENS`, `ITEM_DETAILS`, `AFFIX_CANDIDATE`, `AFFIX_REJECTED`, `AFFIX_MERGE`, `SOCKET_GEM_RUNE_CANDIDATE`, `SOCKET_REGION_LINE`, `TRANSFIGURATION_CANDIDATE`, `ASPECT_MATCH`, `FINAL_IMPORT_FORM`, `FORM_SUBMIT_MAPPING`, `ITEM_LIBRARY_SAVE_READ` i `RUNTIME_CONTRIBUTION`,
