...
Główna tabela używa szerokiego wariantu layoutu i zawiera tylko kolumny decyzyjne: `skillName`, `Kategorie z gry`, `Obrażenia % R1`, `Obrażenia % max drzewo`, `Koszt Wiary`, `Generowanie Wiary`, `Dmg multiplier`, `Dmg bonus`, `Extra hit / component`, `Damage over time`, `Status / debuff`, `Defense / utility` i `Manual review`. Kolumny `Grupa drzewa`, `tags`, `type` oraz `Speed / cooldown` nie są już renderowane w głównej tabeli.
...
Kolumny `Koszt Wiary` i `Generowanie Wiary` są rozdzielone. `Koszt Wiary` pokazuje potwierdzony koszt użycia, np. `Błogosławiony Młot` ma `10`; jeżeli lokalne źródła nie podają kosztu, komórka pokazuje `-`. `Generowanie Wiary` pokazuje bazową generację i potwierdzony dodatek z ulepszenia, np. `Wymach` ma `14; +5 - Generowanie Wiary`, `Święty Pocisk` ma `16; +7 - Generowanie Wiary`, `Starcie` ma `20; +10 - Generowanie Wiary`, a `Natarcie` ma `18`. Te wartości są danymi opisowymi UI i sortowania, nie są używane przez `DamageEngine`.

Kolumny faceted renderują skrócone, opisowe dane z `UpgradeDamageModifier`, jeżeli `safeForRankingDisplay = YES`. `Dmg multiplier` oznacza mnożnik obrażeń, np. `20%[X]`; `Dmg bonus` oznacza addytywny bonus obrażeń, np. `20%[+]`; `Extra hit / component` oznacza osobne źródło obrażeń, dodatkowy hit, powrót pocisku albo dodatkowy łuk; `Damage over time` jest zarezerwowane dla DOT/tick/periodic damage. `Status / debuff` pokazuje tylko konkretną nazwę albo wartość statusu/debuffu, np. `20% - Odsłonięcie`, `Spowolnienie`, `Osąd` albo `Osłabienie`; nie renderuje fraz typu `status / pośredni wpływ` ani `pośredni wpływ`. `Defense / utility` pokazuje konkretną nazwę efektu, np. `Umocnienie`, `Bariera`, `Redukcja obrażeń`, `Blok` albo `Pancerz`, a brak danych w komórkach jest renderowany jako `-`. `Speed / cooldown` nie jest już kolumną głównej tabeli, chociaż dane tego typu mogą pozostać w modelu audytowym.
