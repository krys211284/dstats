...
- Przeistoczenie rozpoznaje wynik `Mnożnik x32% obrażeń (Fizyczne)` jako bonusowy affix Przeistoczenia `PHYSICAL_DAMAGE_MULTIPLIER` z wartością widoczną `32`, statusem prezentacyjnym `Runtime nieaktywny` i bez tworzenia ordinary `ALL_DAMAGE_MULTIPLIER`; analogicznie `Mnożnik x15% wszystkich obrażeń` nie trafia do Przeistoczenia,
- manualny scenariusz akceptacji dla aktualnego realnego przypadku Odłamka Verathiela z danymi broni `2417 / 1884 - 2512 / 1.10` oczekuje `averageWeaponDamage = 2198`, jakości `25/25`, ordinary affixów `+314 obrażeń od broni`, `+270 siły`, `+948 pkt. zdrowia przy trafieniu`, `Mnożnik x15% wszystkich obrażeń` i `+7,5% szansy na trafienie krytyczne` oraz Przeistoczenia `Mnożnik x32% obrażeń (Fizyczne)`; nie wolno przyjmować wartości z cudzych kotwic tej samej sklejonej linii, np. `270` jako `Zdrowie przy trafieniu`, `14` jako `Obrażenia od broni`, `948` jako `ALL_DAMAGE_MULTIPLIER` albo `47,5` jako kryt, i nie wolno tworzyć z socket/gem/rune stats ordinary affixów,
- syntetyczna regresja mechanizmu importu pokrywa wariant bez dedykowanej nazwy/aspektu: `2 417 pkt. obrażeń na sek.`, poprawne `[1 884 - 2 512] pkt. obrażeń za trafienie`, uszkodzone `[884 - 2 512]`, artefakt `II 884 - 2 512`, `1,10 ataku na sekundę`, `25 (+25) jakości`, sklejone `+270 siły +948 pkt. zdrowia przy trafieniu Mnożnik x15% wszystkich obrażeń`, czyste `+314 obrażeń od broni`, uszkodzone `14 obrażeń od broni...`, lore z zerem i słowami o umiejętnościach, które nie może stworzyć `CORE_SKILL_RANKS=0`, oraz `Przeistoczony / Mnożnik x32% obrażeń (Fizyczne)`,
...
- `TEMPERING_FORM` dla hartowania zapisuje co najmniej `definitionId`, `category`, `sourceLine` jeśli istnieje, `storedValue`, `greaterAffix`, `masterworkingQuality`, `perfectedAffix`, `resolvedValue`, `resolvedDisplayText`, `runtimeStatus` i `reason`; log nie może sugerować, że source `+5` jest finalną wartością widoczną, jeżeli UI/runtime używa resolved `+12`,
