...
- pełny zapis itemu w flow importu pokazuje osobno i w czytelnej kolejności nagłówek, linie bazowe / implicit, dodatkowe / sezonowe linie oraz socket / gniazdo; affixy i aspekt / efekt nie są powtarzane w tej sekcji, bo ich źródłem prawdy są odpowiednio tabela ręcznej walidacji affixów oraz blok `Aspekt / efekt` w formularzu potwierdzenia,
...
- import tarczy `Miażdżąca Tarcza Kościanych Łusek` rozpoznaje nazwę w bezpiecznym kontekście tarczy, `itemPower=900`, `itemArmor=1202`, implicity `20,0% szansy na blok [20,0]%` oraz `+100% obrażeń od broni w głównej ręce [100]%`, cztery edytowalne affixy: siłę, odporność na wszystkie żywioły, odporność na Ogień i redukcję obrażeń, z trzema pierwszymi oznaczonymi jako `greaterAffix=true`,
- aspekt tarczy z tekstu `Gdy masz umocnienie, zadajesz obrażenia zwiększone o 61%[x] [45 - 65]%.` mapuje się do `fortify_damage_increased` / `Umocnienie: zwiększone obrażenia`; błędne dopasowanie tego tekstu do `Aspekt Wewnętrznego Spokoju` jest niedozwolone,
- warianty OCR tarczy obejmują osobne cropy dla nazwy, affixów i aspektu, a kontrakt jest pokryty realnym fixture/snapshotem `src/test/resources/items/tarcza.png` oraz `src/test/resources/items/tarcza-windows-ocr-snapshot.txt`,
...
- GUI pokazuje sekcję `Pełny odczyt widocznego itemu` jako czytelny rekord itemu z nagłówkiem, liniami bazowymi / implicit, dodatkowymi liniami i socketem / gniazdem; affixy nie są renderowane w tej sekcji i są edytowane wyłącznie w tabeli ręcznej weryfikacji affixów, a aspekt / efekt jest widoczny tylko w bloku `Aspekt / efekt` formularza potwierdzenia,
...
- render pełnego odczytu itemu jako produktowego podglądu z osobnymi polami nagłówka, implicitami, dodatkowymi liniami i socketem / gniazdem; affixy są dostępne w ręcznej weryfikacji affixów, a aspekt / efekt w pojedynczym bloku `Aspekt / efekt`, nie w sekcji `Pełny zapis itemu`,
