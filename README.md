...
Stare zapisy z polem `enabled` są nadal wczytywane kompatybilnie. `enabled=false`, `qualityCurrent=0`, `qualityMax=25` zostaje `0/25`; `enabled=true`, `qualityCurrent=0`, `qualityMax=25` również zostaje `0/25`; `enabled=true`, `qualityCurrent` z listy realnych progów i `qualityMax=25` zachowuje `qualityCurrent`. Jeżeli starszy zapis ma wartość `0..25` spoza realnych progów, repozytorium wczytuje ją jako `0/25` zamiast zgadywać najbliższy próg. Nowy zapis nie używa `enabled`, a formularz odrzuca niedozwolone wartości czytelnym błędem walidacji z listą progów.
