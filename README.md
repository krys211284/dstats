...
- Aktywna część Molocha: umiejętność rodziny `Moloch` przy co najmniej `8` Animuszu zużywa `8` ładunków, clampuje wynik do minimum `1`, aktywuje buff na `5 s` i dodaje osobny mnożnik obrażeń `+60%[x] / ×1,60` dla umiejętności Molocha. Cast aktywujący także korzysta z buffa; przy obecnym ticku `1 cast/s` buff obejmuje `5` castów, licząc cast aktywujący.
- W tym etapie nie implementowano rozmiaru `+20%`, pancerza `+25%[+]`, zużywania Animuszu przy otrzymywaniu obrażeń ani odzyskiwania Animuszu poza początkową wartością. Nie implementowano też runtime `Adepta`, `Sędziego`, `Zeloty`, Arbitra, Osądu, Ferworu ani Umocnienia z Przysięg.
- Existing sanity bez aktywnego buffa Molocha pozostaje bez zmian: total `21130`, UI `DPS = 2113`, końcowa Wiara `65`. Dla testu `Moloch + Starcie + initialAnimus=8` pierwszy buffowany normalny hit wynosi `3380`, pięć buffowanych i pięć normalnych castów daje total `27465`, UI `DPS = 2746,5`, końcowy Animusz `1`, końcowa Wiara `65`.
