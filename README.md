...
- pełny zapis itemu w flow importu pokazuje osobno i w czytelnej kolejności nagłówek, `Linie bazowe`, dodatkowe / sezonowe linie oraz socket / gniazdo; affixy i aspekt / efekt nie są powtarzane w tej sekcji, bo ich źródłem prawdy są odpowiednio tabela ręcznej walidacji affixów oraz blok `Aspekt / efekt` w formularzu potwierdzenia,
...
- GUI pokazuje sekcję `Pełny odczyt widocznego itemu` wyłącznie jako nieedytowalny podgląd niezdublowanych linii OCR: `Linie bazowe`, dodatkowe/sezonowe linie oraz socket / gniazdo. Ta sekcja nie renderuje kart pól identyfikacyjnych itemu (`Nazwa`, `Typ`, `Rzadkość`, `Ancient`, `Slot`, `Moc przedmiotu`), affixów ani aspektu / efektu; te dane są edytowane wyłącznie w formularzu `Ręczne potwierdzenie itemu`, tabeli affixów i bloku `Aspekt / efekt`,
- `Pełny odczyt widocznego itemu` nie dubluje pól edycyjnych typu itemu: dla broni pola `DPS broni`, min/max/średnia obrażeń za trafienie i `Ataki na sekundę` są edytowane wyłącznie w fieldsecie `Dane broni`, a dla tarczy `Pancerz` jest edytowany wyłącznie w fieldsecie `Dane tarczy`,
...
- GUI po zatwierdzeniu pokazuje `Pełny odczyt zapisany w bibliotece` jako taki sam nieedytowalny podgląd niezdublowanych linii OCR i również nie dubluje pól formularza ani pól typu itemu,
...
- render pełnego odczytu itemu jako produktowego podglądu z osobnymi polami nagłówka, liniami bazowymi, dodatkowymi liniami i socketem / gniazdem; affixy są dostępne w ręcznej weryfikacji affixów, a aspekt / efekt w pojedynczym bloku `Aspekt / efekt`, nie w sekcji `Pełny zapis itemu`,
