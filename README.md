...
- import unikatowej broni kataloguje i prezentuje zwykłe affixy `Obrażenia od broni`, `Zdrowie za zabicie` oraz `Mnożnik obrażeń z upływem czasu`; `Obrażenia od broni` z linii affixu, np. `+134 obrażeń od broni [94 - 157]`, jest niezależne od strukturalnych danych broni `weaponDps`, `weaponDamageMin`, `weaponDamageMax` i `averageWeaponDamage`, a `Mnożnik x16% obrażeń z upływem czasu [15 - 30]%` musi zachować zakres `15 - 30` przez parser, extractor, draft, formularz i finalny HTML. Affixy bez osobnej bezpiecznej ścieżki runtime pozostają wyłącznie prezentacyjne,
