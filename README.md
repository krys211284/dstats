...
Realny fixture po weryfikacji UI: przeistoczona `Miażdżąca Tarcza Kościanych Łusek`, `qualityCurrent=25`, doskonalone hartowanie `maksymalna liczba kumulacji Animuszu`, `outcome=BONUS_TRANSFIGURATION_AFFIX`, `addedTransfigurationAffix=ALL_STATS`, `displayedValue=96`, `valueProvenance=GAME_DISPLAYED_VALUE`, `lockedAfterTransfiguration=true`. Workflow formularza dla tej tarczy: `Stan: Przeistoczony`, `Wynik: Bonusowy affix Przeistoczenia`, `Bonusowy affix: do wszystkich współczynników`, `Wartość widoczna na itemie: 96`, `Pochodzenie wartości: Wartość widoczna w grze`, `Niemodyfikowalny: Tak`, `Pryzmat dostrojenia: Brak`. UI pokazuje summary jako `Przeistoczenie · Bonusowy affix: +96 do wszystkich współczynników · Niemodyfikowalny · Runtime nieaktywny`.
