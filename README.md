...
Import zahartowanej tarczy normalizuje potwierdzony błąd OCR nazwy `Miażdżąca Tarcza Kościanych Łusek`, w tym warianty `L U Sek`, `LUSEK` i końcowe markery gwiazdek. Rozpoznane hartowanie jest usuwane z prezentacji `Dodatkowe / sezonowe linie`, UI importu respektuje limit `1/1` i ukrywa aktywny formularz dodawania, a normalizacja aspektu usuwa znane śmieci OCR typu `Przewiń w dół` z zakresu rolla. Runtime pozostaje bez zmian.
