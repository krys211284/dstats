...
- `Greater Affix` nie pokazuje zakresu rolla: dla `greaterAffix=true` pola `rollRangeMin` i `rollRangeMax` pozostają puste, a UI opisuje to jako poprawny stan `Bez zakresu (Greater Affix)`, nie jako błąd katalogu,
...
Model itemu przechowuje `qualityCurrent`, `qualityMax` oraz opcjonalny `perfectedAffix`. Domyślny stan dla nowych i starszych itemów bez danych Doskonalenia to `qualityCurrent=0`, `qualityMax=25`, `perfectedAffix=null`. Formularz importu i edycji pokazuje sekcję `Doskonalenie`, pole `Jakość aktualna` jako select realnych progów gry, readonly `Jakość maksymalna = 25` oraz status `Runtime aktywny dla potwierdzonych wartości`. Checkbox `Doskonalenie aktywne / Item doskonalony` został usunięty z UI i nie jest źródłem logiki. Backend waliduje `qualityCurrent` wyłącznie jako jeden z progów `0, 3, 6, 9, 12, 15, 17, 20, 21, 25` i wymaga `qualityMax == 25`; inne maksima nie są obsługiwane. Screeny użytkownika przedstawiają wszystkie kroki Doskonalenia, więc w UI nie ma ręcznego inputu liczbowego i nie zakładamy doskonalenia co jeden punkt.
...
Reguły resolved values 2B/2C są ograniczone do potwierdzonych golden values:
...
Nieznane affixy nie są zgadywane. Jeżeli resolver nie ma reguły dla affixu, UI i runtime dla tego affixu używają wartości bazowej oraz UI pokazuje status `Doskonalenie: brak reguły prezentacyjnej dla tego affixu`.
...
- GUI pokazuje sekcję `Pełny odczyt widocznego itemu` jako czytelny rekord itemu z nagłówkiem, sekcją `Linie bazowe`, dodatkowymi liniami i socketem / gniazdem; affixy nie są renderowane w tej sekcji i są edytowane wyłącznie w tabeli ręcznej weryfikacji affixów, a aspekt / efekt jest widoczny tylko w bloku `Aspekt / efekt` formularza potwierdzenia,
...
- katalog affixów zawiera dla Odłamka Verathiela 4 opisowe wpisy: `Obrażenia od broni` z rollem `94 [94 - 157]`, `Maksymalne zdrowie` z rollem `2141 [1831 - 2200]`, `Zdrowie przy trafieniu` z rollem `545 [526 - 632]` oraz `Szczęśliwy traf: zasób podstawowy` z rollem `3` i zakresem `3 - 4`; pełny opis katalogowy tego ostatniego affixu zachowuje stały tekst `maksymalnie 15% szans na odzyskanie +X podstawowego zasobu`,
