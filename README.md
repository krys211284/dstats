...
Import itemu z wielu screenów jest przeznaczony dla przewijanego tooltipa, który nie mieści się na jednym ekranie. Formularz pozwala wybrać `1..5` obrazów tego samego itemu; screeny trzeba dodać w kolejności od góry tooltipa do dołu. Importer wykonuje OCR każdego obrazu osobno, scala tekst w kolejności uploadu, usuwa overlap oraz linie UI-only typu `Przewiń w dół`, `Przewiń do góry`, akcje ekwipunku, sprzedaż, trwałość i porównawcze `(Wytrzymałość: +4,6%)`, a parser dostaje jeden logiczny opis itemu.

Realny przykład wieloscreenowy to przeistoczona `Miażdżąca Tarcza Kościanych Łusek` z jakością `25/25`. Dla takiego itemu wartości zwykłych affixów i hartowania widoczne w tooltipie są wartościami finalnymi po Doskonaleniu, więc importer odtwarza source values zapisu: np. `+270 siły` trafia jako source `225` GA, odporności `+588/+945` jako `490/787` GA, `14,3% redukcji obrażeń` jako source `11,4`, a `+12 do maksymalnej liczby kumulacji Animuszu` jako source hartowania `defense_max_animus=5` GA z `perfectedAffix=TEMPERING_AFFIX:defense_max_animus`. Bonus `+96 do wszystkich współczynników` jest affixem Przeistoczenia `ALL_STATS` z `displayedValue=96` i `valueProvenance=GAME_DISPLAYED_VALUE`, nie zwykłym affixem itemu. Przeistoczenie pozostaje runtime nieaktywne.
