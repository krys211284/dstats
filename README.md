...
Główna tabela używa szerokiego wariantu layoutu i zawiera tylko kolumny decyzyjne: `skillName`, `Kategoria`, `tags`, `type`, `Obrażenia % R1`, `Obrażenia % max drzewo`, `Dmg multiplier`, `Dmg bonus`, `Extra hit / component`, `Damage over time`, `Status / debuff`, `Resource`, `Speed / cooldown`, `Defense / utility` i `Manual review`. Kolumna `Kategoria` jest prezentacyjnym tierem/kategorią z lokalnego rejestru i opisów: np. `Wymach` ma `Adept`, bo lokalny Markdown `paladin_basic_skill_registry_final.md` potwierdza tagi `Podstawowe, Adept`. Techniczny `skillGroup` pozostaje w danych i filtrze, ale nie jest już osobną szeroką kolumną główną.
...
Tabela jest sortowalna kliknięciem w nagłówki kolumn. Widok używa parametrów `sort=<columnKey>` i `direction=asc|desc`; aktualnie sortowany nagłówek ma `aria-sort`. Obsługiwane klucze sortowania to `skillName`, `skillCategory`, `type`, `baseDamageRank1`, `baseDamageTreeMax`, `maxDamageMultiplierPercent`, `maxDamageBonusPercent`, `maxExtraHitOrComponentPercent`, `maxDamageOverTimePercent`, `damageProfile`, `hasStatusDamageEnabler`, `hasResourceGeneration`, `hasCooldownOrCastSpeed`, `hasDefenseOrUtility`, `hasManualReviewUpgrade` i `tags`. Numeryczne sort keys dla modyfikatorów używają największej wartości procentowej w danej kategorii, bez sumowania wielu wpisów. Domyślne sortowanie to `baseDamageTreeMax desc`; wpisy bez prostej wartości liczbowej trafiają za wpisami liczbowymi, a remisy są rozstrzygane przez `skillName asc`. Dotychczasowy filtr `Metryka rankingu` został zastąpiony sortowaniem po nagłówkach.
...
- ekran renderuje kolumny `Kategoria`, `tags`, `Dmg multiplier`, `Dmg bonus`, `Extra hit / component`, `Damage over time`, `Status / debuff`, `Resource`, `Speed / cooldown`, `Defense / utility` i `Manual review` zamiast szerokich kolumn `grupa_1`, `grupa_2`, `grupa_3`,
