...
- `ItemOcrBlockSegmenter` dodaje kandydackie segmenty z długich bloków OCR na podstawie kotwic z `AffixRegistry`, `ApplicationTemperingAffixRegistry`, `TransfigurationAffixCatalog` i `ApplicationAspectRegistry`; oryginalna linia pozostaje zachowana jako diagnostyczne źródło, a segmenty są dodatkowymi kandydatami, nie destrukcyjną zamianą,
...
- `FullItemRead` reprezentuje pełniejszy odczyt widocznego itemu z OCR: nazwę, typ / slot, rzadkość, moc przedmiotu, base stat, linie bazowe, właściwe affixy, aspekt / moc, gniazda i inne zachowane linie,
- `ItemImportDetails` reprezentuje strukturalnie potwierdzane dane itemu: `itemName`, `itemType`, `itemRarity`, `isAncient`, `equipmentSlot`, `itemPower`, osobne pola broni `weaponDps`, `weaponDamageMin`, `weaponDamageMax`, `averageWeaponDamage`, `attacksPerSecond`, osobne pole tarczy `itemArmor` oraz opisowy `uniqueEffectText`,
- `uniqueEffectText` zachowuje semantycznie istotne tokeny liczbowe z OCR efektu/aspektu: liczby, procenty, nawiasy kwadratowe, `[x]` i mnożniki typu `80%[x]`; uszkodzone, ale jednoznaczne tokeny mnożnika, np. `809ó[x]`, mogą zostać znormalizowane do `80%[x]`, natomiast brak takiego tokenu w finalnym tekście nie może wynikać z wyboru uboższego wariantu OCR, jeżeli bogatszy wariant opisuje ten sam efekt,
- jeżeli OCR pasuje do znanego aspektu albo unikatowego efektu z `ApplicationAspectRegistry`, finalny `effectText` / `uniqueEffectText` w formularzu ma używać kanonicznego tekstu z katalogu; roll albo mnożnik z OCR może zostać podstawiony tylko wtedy, gdy token jest realnie obecny i bezpiecznie rozpoznany, a brak pewnego rolla nie uprawnia do zgadywania wartości,
...
- import pojedynczego screena używa typowanego scalania `ItemScreenshotTextMerger.mergeTextVariantsTyped(...)` jako autorytatywnego wejścia dla `FullItemRead`, wizualnej kolejności ordinary affixów oraz danych socket/gem/rune; nie istnieje już kontrakt ścieżki `single-no-merge`, w której finalny odczyt powstaje wyłącznie z surowych wariantów OCR,
- surowe per-variant parse'y pojedynczego screena pozostają pomocnicze dla pól foundation, `ItemImportDetails`, uzupełnienia czytelniejszego selected text/value/range i diagnostyki, ale nie mogą ustawiać finalnej kolejności ordinary affixów, jeżeli typed merge ma semantycznie pasujący klucz. W takim przypadku surowy kandydat może wygrać tekstem albo zakresem, a typed line pozostaje kotwicą wizualną i źródłem pozycji,
...
- bieżący `AspectRegistry` jest zalążkowym katalogiem znanych aspektów potrzebnym do stabilizacji importu; nie udaje pełnej produkcyjnej bazy wszystkich aspektów Diablo 4 i zawiera wyłącznie świadomie dodane definicje, obecnie `inner-calm` / `Aspekt Wewnętrznego Spokoju` dla zgodnych slotów ręki dodatkowej, `fortify_damage_increased` / `Umocnienie: zwiększone obrażenia` jako opisowy aspekt legendarny `DESCRIPTIVE_ONLY` dla tarczy oraz `verathiel_shard` / `Odłamek Verathiela` jako aspekt unikatowy `UNIQUE` dla `MAIN_HAND`,
...
- `ApplicationAspectRegistry` może dostarczyć kanoniczny opis aspektu / efektu unikatowego do formularza importu, ale sama obecność liczbowego mnożnika w opisie aspektu `DESCRIPTIVE_ONLY` nie aktywuje DPS,
...
- obecność liczby i mnożnika w `uniqueEffectText`, np. `80%[x]` dla efektu `DESCRIPTIVE_ONLY`, jest wyłącznie zachowaniem tekstu do ręcznej weryfikacji i nie oznacza aktywacji mnożnika w `Damage Engine`, manual simulation, searchu ani projekcji current build,
...
- `FullItemReadLine` nie jest już samym `type/text`: każda linia ma zachować `FullItemReadLineSource` z metadanymi źródła i segmentu OCR: `sourceScreenIndex`, `sourceVariantId`, `sourceLineOrder`, `sourceSegmentStart`, `sourceSegmentEnd`, `sourceRawLine`, `parentRawLine`, `sourceRegion`, `visualBlockOrder`, `visualSourceLineOrder`, `visualSegmentStart`, `visualSegmentEnd` oraz `visualSourceText`; stare testy i ręcznie tworzone linie używają bezpiecznego źródła `UNKNOWN`, ale pipeline OCR nie może tych pól wyzerować,
- `ItemScreenshotTextMerger` i `ItemImageImportTextParser` mogą skleić wieloliniowy affix do jednej linii roboczej, ale taka ekspansja musi zachować metadane pierwszego pasującego segmentu OCR, żeby `+15,0% szansy na trafienie` + `krytyczne [12,0]%` nadal było jednym affixem, a jednocześnie miało realną kotwicę pozycyjną,
- `ImportedItemAffix` reprezentuje pojedynczy edytowalny affix itemu jako strukturę: typ z katalogu, nazwa / etykieta, wartość, jednostka, `greaterAffix`, kolejność prezentacji wybranego kandydata, surowa linia OCR, źródło `OCR`, `MANUAL` albo `CORRECTED`, a także osobną kotwicę wizualną `visualSourceText` / `visualDisplayOrder` oraz stan `greaterAffixConfirmationRequired`,
- deduplikacja kilku kandydatów tego samego affixu wybiera semantycznie najlepszy kandydat wartości/tekstu, np. wariant z pewnym zakresem rolla zamiast wcześniejszego szumu OCR, ale nie może przy tym zgubić najlepszej wiarygodnej kotwicy pozycyjnej dla tej samej semantycznej linii; finalny affix może więc mieć `selectedValueSource` z czystszego kandydata oraz `visualAnchorSource` z wcześniejszego, lokalnego i zgodnego semantycznie kandydata,
- kotwica wizualna może pochodzić z kandydata tekstowo gorszego tylko wtedy, gdy dotyczy tego samego affixu, tej samej wartości / semantycznej linii, tej samej sekcji i nie pochodzi z obcej kotwicy ani cudzej liczby; noisy candidate nie może zmieniać wartości finalnego affixu, ale może ustawić `visualDisplayOrder` i przenieść lokalny marker GA,
- ordinary visual order nie może być liczony z kolejności `LinkedHashMap`, z kolejności `mergedLines.values()` ani z prostego licznika iteracji po `fullItemRead.getLines()`; podstawą jest indeks linii źródłowej w finalnym `FullItemRead`, a jeżeli jedna typowana albo surowa linia zawiera kilka affixów, sortowanie w tym bloku idzie po `visualDisplayOrder` / `segmentStart`,
- formularz importu i edycji renderuje ordinary affixy w wizualnej kolejności z tooltipa: najpierw po linii wskazanej przez `visualSourceText` w `FullItemRead`, potem po `visualDisplayOrder`, a następnie po `displayOrder` wybranego kandydata. Renderer nie może przestawiać listy alfabetycznie, po enumie, score, map insertion order, nazwie ani po typie affixu,
- `value` / `displayedValue` affixu importowanego z tooltipa jest wartością widoczną na itemie i pozostaje wartością runtime; `referenceValue` / `baseValue` jest metadanym informacyjnym pochodzącym z wiarygodnego OCR bracketu albo z jednoznacznego reverse-resolve Doskonalenia,
- jeżeli OCR dostarcza wiarygodny bracket/reference, ma on pierwszeństwo przed reverse-resolve; jeżeli bracket jest uszkodzony albo nieobecny, reverse-resolve może uzupełnić `referenceValue` tylko dla obsługiwanego typu i jednoznacznego wyniku, a w przeciwnym razie `referenceValue` pozostaje puste,
- wartości i `referenceValue` affixów rang umiejętności muszą pochodzić z OCR albo jednoznacznego reverse-resolve; importer nie może tworzyć rang z katalogu, nie może naprawiać uszkodzonego single-value bracketu do konkretnej liczby bez podstawy i nie może zmieniać widocznej wartości,
...
- ordinary / zwykłe staty podstawowe itemu mają twardy limit 4 pozycji. Piąta stat-like linia nie może trafić do `ImportedItemAffix` tylko dlatego, że pasuje do `AffixRegistry`; po osiągnięciu limitu importer musi użyć regionu i katalogów domenowych (`ApplicationTemperingAffixRegistry`, `TransfigurationAffixCatalog`, socket/gem/rune, dataOnly/ignored) i zalogować decyzję klasyfikacji,
- cap ordinary affixów jest liczony po finalnym sortowaniu według wizualnej kolejności po deduplikacji. Wariant OCR odrzucony na rzecz lepszego kandydata wartości nie może wypychać prawidłowego affixu spośród pierwszych czterech pozycji, ale może pozostać kotwicą wizualną, jeżeli jest lokalny, semantycznie zgodny i ma wiarygodną pozycję w tooltipie,
- kandydat pasujący do katalogu hartowania nie może rezerwować miejsca w ordinary capie, jeżeli cztery ordinary sloty można wypełnić kandydatami bez lepszej klasyfikacji domenowej; w takiej sytuacji importer odracza go i szuka właściwej linii hartowania po wizualnym końcu ordinary setu,
- jeżeli po czterech ordinary affixach kolejna stat-like linia pasuje do katalogu hartowania, trafia do sekcji `Hartowanie` jako `DATA_ONLY`, nie jest duplikowana w ordinary i nie zwiększa liczby ordinary ponad 4. Przykładem mechanizmu jest `+...% szansy na trafienie krytyczne`: przed limitem może być kandydatem zwykłego affixu zgodnie z regionem, ale po limicie i przy dopasowaniu katalogu hartowania jest klasyfikowana jako hartowanie,
- hartowanie przechowuje finalne `sourceLine` / display source z wybranego kandydata, a nie z pierwszej linii pasującej po nazwie definicji; gdy artefakt OCR `+47,5%...` zostaje odrzucony, a finalnym kandydatem jest `+7,5%...`, debug i UI muszą wskazywać źródło `+7,5%...`,
- `greaterAffix` jest trwałym polem boolean modelu affixu, niezależnym od surowej linii OCR i od prezentacyjnej gwiazdki w UI,
- OCR oznacza `greaterAffix=true`, gdy rozpoznany edytowalny affix zaczyna się od markera `*`, `★`, `⭐`, `✦`, `•` albo pokrewnego stabilnego glifu początku linii; jeżeli ten sam affix występuje w kilku wariantach OCR, deduplikacja zachowuje wariant z gwiazdką,
- marker GA jest przypisywany per line / per segment, nie globalnie dla całego itemu i nie na podstawie samej wartości. Jeżeli marker jest widoczny lokalnie przy linii affixu albo przy jej zachowanej kotwicy wizualnej, `greaterAffix=true` musi przejść do finalnego affixu i checkboxa `Gwiazdka`; dedup replacement nie może zgubić markera tylko dlatego, że czystszy selected value candidate nie zawiera gwiazdki,
- marker GA z `parentRawLine` albo `visualSourceText` może zostać przypisany do finalnego selected affixu tylko wtedy, gdy leży przed offsetem segmentu tego affixu, w tym samym ordinary visual blocku i bez separatora sekcji między markerem a segmentem; najbliższy lokalny marker wygrywa, a sam globalny marker bez lokalnego dopasowania nie zaznacza checkboxów,
- stany GA są jawne: lokalnie potwierdzony marker ustawia `greaterAffix=true` bez wymagania potwierdzenia; marker z wiarygodnej kotwicy wizualnej albo prawdopodobny marker blokowy w typowanym ordinary blocku ustawia `greaterAffix=true` oraz `greaterAffixConfirmationRequired=true`, więc checkbox jest zaznaczony i wiersz pokazuje `GA do potwierdzenia`; niepowiązany globalny marker pozostawia checkbox pusty i ustawia stan wymagający potwierdzenia; brak markera pozostaje zwykłym `false`,
- jeżeli marker GA istnieje w OCR, ale nie da się go bezpiecznie powiązać z konkretnym affixem, debug / UI ma sygnalizować stan wymagający potwierdzenia zamiast cichego `false`, a Doskonalenie 25/25 zapisuje nieznane źródło `REQUIRES_CONFIRMATION / ambiguous_ga_markers`. Taki stan musi być widoczny także przy sekcji affixów jako ostrzeżenie oraz per-affix jako informacja `GA do potwierdzenia`; pusty checkbox w tym stanie nie oznacza pewnego `false`, tylko brak bezpiecznego lokalnego przypisania,
- `Greater Affix` zachowuje zakres rolla, jeżeli OCR dostarczył stabilne `rollRangeMin` / `rollRangeMax`; tylko rzeczywisty brak zakresu jest opisywany w UI jako `Bez zakresu (Greater Affix)`, nie jako błąd katalogu,
- parser ma zachować bracketowy zakres rolla, jeżeli OCR go odczytał, także gdy OCR doda techniczny znak przed nawiasem, np. `+[107 - 121]`, gdy roll jest stały i zapisany jako `[+300]`, gdy zakres procentowy ma przecinek dziesiętny, np. `[6,5 - 8,5]%`, gdy OCR zamieni zamykający nawias na `1`, np. `[15 - 301%`, oraz gdy linie OCR są skondensowane w jeden długi tooltip,
- segmentacja skondensowanych bloków OCR używa katalogowych kotwic affixów, hartowania, przeistoczeń i aspektów do dodawania kandydackich linii; nie może przenosić jednego bracketu do wielu affixów ani usuwać oryginalnej linii źródłowej,
- importer obsługuje `orphan roll ranges`: zakres odczytany jako osobna linia, np. `[15 - 30]%`, albo jako kontynuacja bez własnej wartości, np. `czasu [15 - 30]%`, może zostać dopięty wyłącznie do bezpośrednio poprzedzającego affixu bez zakresu, jeśli nie ma drugiego możliwego nierozwiązanego affixu w sąsiedztwie i jednostka procentowa nie przeczy wartości affixu; jeden taki zakres jest używany tylko raz,
- importer nie uzupełnia brakującego zakresu wyłącznie z katalogu affixów; jeżeli OCR nie zawiera zakresu albo przypisanie orphan range jest niejednoznaczne, affix zostaje bez zakresu zamiast zgadywania,
- przeistoczenia są rozpoznawane po tekście / semantycznej kotwicy z `TransfigurationAffixCatalog`, np. `do wszystkich współczynników -> ALL_STATS`; displayed value może być dowolna, bo może zawierać Doskonalenie, GA, jakość albo przyszłą zmianę gry, a katalogowy zakres nie jest filtrem blokującym import,
- `Niezniszczalność` wykryta na itemie oznaczonym jako `Przeistoczony` jest wynikiem Przeistoczenia / Kostki Horadrimów (`INDESTRUCTIBLE`), bez wartości liczbowej, ze statusem `Runtime nieaktywny`. Nie jest ordinary affixem, socket statem ani aktywną mechaniką DPS,
- `Mnożnik x...% obrażeń (Fizyczne)` pozostaje affixem Przeistoczenia tylko wtedy, gdy linia należy do regionu Przeistoczenia. Jeżeli linia występuje w regionie socket/gem/rune, po markerze gniazda/gema/runy albo przy silniejszym kontekście `Niezniszczalność` z Przeistoczenia, jest klasyfikowana jako `SocketGemRuneStat` z `damageType=PHYSICAL`, `runtimeStatus=DATA_ONLY` i nie może zastąpić bardziej wiarygodnego wyniku Przeistoczenia `Niezniszczalność`; fallback socket/gem/rune nie wymaga, żeby parser wcześniej nadał linii typ `SOCKET`,
- `ALL_DAMAGE_MULTIPLIER` jest affixem opisowym/data-only bez aktywnego DPS. UI pokazuje czytelny tekst tooltipa, np. `Mnożnik x15% wszystkich obrażeń`, a edytowalne pole przechowuje samą wartość liczbową `15`; brak reguły Doskonalenia dla tego affixu nie może być prezentowany jako błąd danych w wierszu affixu,
- source range przeistoczenia, np. `75 - 100`, jest metadanym odczytanym z OCR, jeśli występuje; nie wolno odrzucać `+115 pkt. do wszystkich współczynników [75 - 100]` tylko dlatego, że `115` wykracza poza katalogowy zakres źródłowy,
- dopasowanie tekstu do `AffixRegistry` nie wystarcza do utworzenia ordinary affixu; linia musi dodatkowo należeć do zwykłego regionu affixów itemu. Linie z dolnego regionu tooltipa po aspekcie / efekcie albo w regionie gniazd, gemów i run nie tworzą `ImportedItemAffix`, nawet jeżeli tekst zawiera np. `+150 siły`, `+120 inteligencji` albo inną frazę pasującą do katalogu affixów,
- `ItemScreenshotTextMerger` musi zachować region źródłowy wariantu OCR i surowej linii przed `splitLogicalLines` oraz przed `canonicalKnownAffixLine`; każdy segment wycięty z raw/source line zachowuje `parentLineRegion`, `derivedFromSplit=true`, offsety `segmentStart` / `segmentEnd` oraz granice pierwszego efektu aspektu, socket/gem/rune i lore/vendor requirement w tej samej raw line. `parentLineRegion` jest wskazówką diagnostyczną, a nie blokadą całej linii: efektywny region segmentu wynika z lokalnej kotwicy i pozycji segmentu względem granic. Zwykły affix, przeistoczenie albo hartowanie stojące przed pierwszym efektem aspektu pozostaje odpowiednio `ORDINARY_AFFIX_REGION`, `TRANSFIGURATION_REGION` albo `TEMPERING_REGION`, nawet jeżeli dalsza część tej samej raw line zawiera efekt, socket albo lore. Segment statystyki z efektywnego regionu `ASPECT_EFFECT_REGION`, `SOCKET_GEM_RUNE_REGION`, `LORE_VENDOR_REQUIREMENT_REGION` albo innego non-ordinary regionu nie może zostać skanonizowany do klucza `affix:*` ani trafić do `SCREEN_MERGER_OUTPUT` jako zwykły affix. Parser zachowuje własną klasyfikację regionu jako drugą linię obrony dla tekstu, który nie przeszedł przez merger, ale sam merger nie może promować gem/rune stats do ordinary affixów,
- wieloscreenowy import musi używać typowanego wyniku merge, a nie samego `String`: linia rozpoznana w per-screen merge jako `socket-stat:*` zachowuje `canonicalKey`, kategorię `SOCKET_GEM_RUNE`, `sourceRegion`, raw line, wariant, offsety segmentu, `localAnchorType`, opcjonalny `matchedType`, sparsowane `value` i `runtimeStatus=DATA_ONLY` do multi-screen merge oraz parsera. Nie wolno serializować `socket-stat:*` do gołego tekstu typu `+150 siły` i ponownie przepuszczać go przez `canonicalKnownAffixLine` jako kandydat `affix:*`,
- jedna raw OCR line może zawierać wiele statów socket/gem/rune, np. `+150 siły +120 siły` albo `+120 inteligencji +500 pkt. pancerza`. Deduplikacja takich statów nie może używać samego `sourceRawLine`, samego `rawLineOrder` ani samego `matchedAffixType`; occurrence musi rozróżniać co najmniej położenie segmentu (`segmentStart` / `segmentEnd`) i tekst / wartość. Dwa staty tego samego typu i tej samej wartości z różnych segmentów pozostają osobnymi wykrytymi statami, dopóki model gniazd nie zdecyduje inaczej limitem liczby gniazd,
- socket/gem/rune stats mogą dodatkowo mieć klucz semantyczny do deduplikacji duplikatów OCR tego samego wystąpienia, np. `PHYSICAL_DAMAGE_MULTIPLIER|32|PHYSICAL|DATA_ONLY`. Taka deduplikacja nie zastępuje occurrence dla zwykłych gem statów: dwa realne segmenty `+120 siły +120 siły` pozostają dwoma statami, natomiast czysty `Mnożnik x32% obrażeń (Fizyczne)` wygrywa ze sklejką zawierającą lore, np. `... TO OSTRZE W RĘKACH...`,
- deduplikacja socketów nie zastępuje lokalnego value bindingu. Dla `Mnożnik x...% obrażeń (Fizyczne)` wartość musi być związana lokalnie z anchorami `Mnożnik`, `x`, `%`, `obrażeń` i `Fizyczne`; liczby z wcześniejszego unikatowego efektu, zakresu rolla albo sklejki typu `100%[x] [70 - 100]`, `0 170` nie mogą utworzyć socket statów `x100`, `x70` ani `x170`. Jeżeli w tej samej raw line występuje unique effect i późniejszy socket multiplier, extractor ignoruje liczby sprzed socket anchora i wybiera liczbę po lokalnym `x`,
- dla regresji z Odłamkiem Verathiela fizyczny socket/gem/rune multiplier `Mnożnik x32% obrażeń (Fizyczne)` pozostaje `SocketGemRuneStat` z `runtimeStatus=DATA_ONLY`; liczby efektu unikatowego i artefakty OCR typu `x170` nie mogą go zastąpić. Dla Dziedzica Zatracenia statystyki gemów/run pozostają w sekcji `Gniazda` jako wykryte dane socket/gem/rune, a nie jako ordinary affixy,
- wszystkie ścieżki w mergerze tworzące `CanonicalLineCandidate("affix:*", ...)` muszą przechodzić przez region-aware guard; fallback po `canonicalKnownLine` nie może wynosić prostych statów ani bazowych linii z dolnego/lore regionu jako zwykłych linii, które parser później potraktuje jak ordinary affix,
- rozpoznawanie regionu socket / gem / rune jest generyczne: nie jest specjalnym wyjątkiem dla `STRENGTH` ani dla `Dziedzica Zatracenia`; krótka linia statystyki zaczynająca się od `+` po regionie aspektu / efektu albo w sekcji gniazd zostaje potraktowana jako linia gniazda/gema/runy, o ile nie jest znaną linią zwykłego affixu potrzebną do rekonstrukcji ordinary regionu po scaleniu screenów, hartowania, przeistoczenia albo aspektu,
- parser broni normalizuje wewnętrzne spacje w liczbach jako separator tysięcy tylko w obrębie pojedynczej liczby: `1 390` daje `1390`, `2 018` daje `2018`, a zakres `[1 390 - 2 018] pkt. obrażeń za trafienie` daje `weaponDamageMin=1390`, `weaponDamageMax=2018`, `averageWeaponDamage=1704`; nie wolno doklejać cyfr z poprzednich pól tooltipa, np. DPS broni,
- regresje importu chronią tarcze przed utratą zakresów rolli po normalizacji skondensowanego OCR: `Tarcza Burzy Księżycowego Szału` zachowuje zakresy `150 - 180`, `6,5 - 8,5`, `11,0 - 15,0` oraz zakres znany dla `Redukcji czasu odnowienia` przy zachowaniu source values po Doskonaleniu,
...
- import unikatowej broni kataloguje i prezentuje zwykłe affixy `Obrażenia od broni`, `Zdrowie za zabicie` oraz `Mnożnik obrażeń z upływem czasu`; `Obrażenia od broni` z linii affixu, np. `+134 obrażeń od broni [94 - 157]`, jest niezależne od strukturalnych danych broni `weaponDps`, `weaponDamageMin`, `weaponDamageMax` i `averageWeaponDamage`, a `Mnożnik x16% obrażeń z upływem czasu [15 - 30]%` musi zachować zakres `15 - 30` przez parser, extractor, draft, formularz i finalny HTML także w single-screen OCR, gdy tooltip zawija tę linię jako `Mnożnik x16% obrażeń z upływem` + `czasu [15 - 30]%` albo jako `Mnożnik x16% obrażeń z upływem czasu` + osobna linia `[15 - 30]%`. Przy scalaniu wariantów OCR ten sam affix z rzeczywistym zakresem rolla wygrywa z wariantem bez zakresu przed zwykłą punktacją jakości linii. Affixy bez osobnej bezpiecznej ścieżki runtime pozostają wyłącznie prezentacyjne,
...
- po zapisie itemu kanonicznym źródłem user-facing danych biblioteki i edycji jest `SavedImportedItem.details` / `ItemImportDetails`, `SavedImportedItem.displayName`, finalny `selectedAspectId` oraz zapisana lista affixów,
- `FullItemRead` pozostaje surowym, diagnostycznym odczytem OCR; nie wygrywa z `ItemImportDetails.itemName` przy nazwie w głównym UI biblioteki ani w formularzu edycji,
- edycja zapisanego itemu nie uruchamia ponownie OCR i odtwarza formularz z danych zapisanych w bibliotece, dzięki czemu zapis bez zmian nie kasuje danych broni, aspektu, affixów ani zakresów rolla,
...
- dla tarczy import zapisuje `itemArmor` jako pancerz i renderuje osobny blok `Dane tarczy`; sekcja `Dane broni` jest renderowana wyłącznie dla itemów broni i nie może pojawiać się jako pusty blok dla tarczy albo innych niebroni,
- linie bazowe to właściwości bazowe typu itemu, np. redukcja blokowanych obrażeń, szansa bloku albo obrażenia od broni w głównej ręce na tarczy; są prezentowane osobno od affixów i w UI nie używają etykiety `implicit`,
...
- raw OCR efektu aspektu jest danymi pomocniczymi: może zostać zachowany w modelu/debugu albo pokazany w ścieżce ręcznej diagnostyki, ale dla dopasowanego aspektu katalogowego nie jest widocznym tekstem głównego formularza,
- socket / gniazdo jest osobną sekcją prezentacyjną i nie może przejmować tekstu aspektu ani affixów.
- statystyki wykryte w regionie socket / gem / rune nie są ordinary affixami i nie mogą znikać po odfiltrowaniu z ordinary affixów. Merger zachowuje je jako typowane linie `socket-stat:*`, multi-screen merge przenosi metadata bez reprocessingu do `affix:*`, parser klasyfikuje je jako `FullItemReadLineType.SOCKET`, a formularz zapisuje je w `ItemSocketing` jako `DETECTED_STAT` z modelem `SocketGemRuneStat`. Taki stat ma `displayText`, `normalizedText`, bezpiecznie sparsowane `value`, opcjonalny `matchedAffixType`, opcjonalny `damageType`, `sourceLine`, `sourceRegion=SOCKET_GEM_RUNE_REGION` i `runtimeStatus=DATA_ONLY`; pozostaje prezentacyjny i runtime inactive,
- jeśli nie ma pewnej jawnej liczby gniazd, ale wykryto stat socket/gem/rune, UI nie może pokazywać wyłącznie mylącego `Liczba gniazd: 0`. Wykryty stat jest prezentowany w sekcji `Gniazda` jako zajęte gniazdo / `DETECTED_STAT`, a liczba gniazd wynika co najmniej z `occupiedSocketCount` ograniczonego aktualnym modelem,
- liczba gniazd nie może zależeć wyłącznie od linii `Puste gniazdo`: `occupiedSocketCount` wynika z wykrytych niezależnych statów socket/gem/rune, `emptySocketCount` z pustych gniazd, a `totalSocketCount = emptySocketCount + occupiedSocketCount` z limitem aktualnie wspieranego modelu. Jeżeli tooltip pokazuje dwa zajęte gniazda przez staty `+150 siły` i `+120 siły`, oczekiwane jest `occupiedSocketCount=2`, `emptySocketCount=0`, `totalSocketCount=2`. Jeżeli tooltip pokazuje `Puste gniazdo` oraz jeden wykryty stat, oczekiwane jest `emptySocketCount=1`, `occupiedSocketCount=1`, `totalSocketCount=2`,
- UI importu i edycji pokazuje wykryte staty socket/gem/rune w sekcji `Gniazda`, ustawia `Liczba gniazd` na `totalSocketCount`, pokazuje teksty statów przy odpowiednich gniazdach i oznacza je jako `Runtime nieaktywny`. `DETECTED_STAT` jest prezentowany jako czytelny wykryty stat domenowy, np. `Wykryty stat gema/runy: +150 siły`, a nie jako pusty widoczny select `Gem`. Mechanizm nie jest wyjątkiem dla `STRENGTH` ani dla `Dziedzica Zatracenia`; różne staty z regionu socket/gem/rune, np. inteligencja, zręczność, siła woli albo pancerz, są zachowywane jako socket data i nie trafiają do ordinary affixów,
- runtime gemów/run pozostaje nieaktywny do czasu osobnego, zweryfikowanego kontraktu: `CurrentBuildRuntimeInputResolver` nie dolicza wykrytych socket/gem/rune stats do siły, inteligencji, zręczności, siły woli, pancerza, odporności, szansy kryta ani DPS. Debug runtime może logować je jako `ignoredForRuntime=true` z powodem `socket/gem/rune runtime is not implemented/verified`.
...
- normalny ekran importu itemu i ekran edycji itemu nie renderują technicznego `Pełnego zapisu itemu` ani raw dumpu `Socket / gniazdo`; `FullItemRead` pozostaje w modelu, hidden fieldach, submit/roundtrip i repozytorium, ale użytkownik widzi dane w uporządkowanych sekcjach domenowych: `Ręczne potwierdzenie itemu` albo `Ręczna edycja itemu`, `Aspekt / efekt`, `Ręczna weryfikacja affixów`, `Hartowanie`, `Doskonalenie`, `Przeistoczenie / Kostka Horadrimów` i `Gniazda`,
- sekcja `Gniazda` w imporcie i edycji jest uporządkowaną sekcją domenową: zwykły `GEM` pokazuje select gema i efekt, `EMPTY` pokazuje prosty pusty stan, a `DETECTED_STAT` pokazuje kartę wykrytego statu z tekstem statystyki i statusem `Runtime nieaktywny`; techniczne hidden fields zachowują roundtrip, ale nie mogą tworzyć pustych widocznych kolumn ani chaosu layoutu,
- przycisk `Zapisz zmiany` na ekranie edycji itemu jest normalną akcją formularza w osobnym kontenerze akcji po sekcji `Gniazda`; nie może znajdować się strukturalnie ani wizualnie wewnątrz fieldsetu `Gniazda`, wiersza `socketing-row` ani karty detected statu,
...
- sekcja aspektu w imporcie pokazuje jedno widoczne miejsce `Aspekt / efekt`: select `selectedAspectId` oraz jedno pole `Treść efektu`; typ aspektu, status runtime, sugestia OCR, confidence i `Odczyt OCR efektu` pozostają danymi modelu / debug, ale nie są widocznym tekstem głównego formularza dla dopasowanego aspektu katalogowego Odłamka Verathiela,
...
- jeżeli item ma już 4 zwykłe affixy, UI pokazuje komunikat `Limit affixów dla tego przedmiotu został wykorzystany.` i nie renderuje formularza `Dodaj affix`, pól `Typ affixu`, `Wartość`, `Greater Affix` ani przycisku dodawania,
...
- import tarczy jest generyczny dla struktury tooltipa Diablo 4 i nie jest whitelistą nazw itemów; parser zbiera górne linie nazwy przed linią typu/rzadkości/mocy/base stat, dzięki czemu nazwa zawierająca słowo `Tarcza`, np. `Miażdżąca Tarcza Kościanych Łusek`, pozostaje nazwą, a nie linią typu,
- snapshot-only regresja `src/test/resources/items/tarcza-koscianych-lusek-ocr-snapshot.txt` pokrywa przypadek Kościanych Łusek: `itemPower=900`, zdeduplikowany `itemArmor=1202`, linie bazowe `20,0% szansy na blok [20,0]%` oraz `+100% obrażeń od broni w głównej ręce [100]%`, cztery edytowalne affixy i opisowy aspekt umocnienia; plik ten nie jest deklarowany jako OCR z `src/test/resources/items/tarcza.png`, bo ten obraz przedstawia Nestorską Egidę Wewnętrznego Spokoju,
- import tarczy Kościanych Łusek zachowuje `greaterAffix=true` dla linii oznaczonych gwiazdką / markerem w dowolnym wariancie OCR; dodatkowa heurystyka bez markera jest ograniczona do wyraźnego kontekstu tej tarczy i stabilnych linii `+225 siły`, `+787 do odporności na: Ogień` oraz `+490 do odporności na wszystkie żywioły`,
- w przykładowej tarczy Kościanych Łusek tylko `Redukcja obrażeń` nie jest `Greater Affix` i pokazuje normalny zakres rolla `11,0 - 15,0`; trzy linie `Siła`, `Odporność na Ogień` i `Odporność na wszystkie żywioły` mają puste zakresy jako poprawną semantykę `Greater Affix`,
- aspekt tarczy z tekstu `Gdy masz umocnienie, zadajesz obrażenia zwiększone o X%[x] [45 - 65]%.` mapuje się do `fortify_damage_increased` / `Umocnienie: zwiększone obrażenia`; konkretny roll `X` pochodzi z odczytu itemu, może być naprawiony z OCR typu `610[x] -> 61`, `610/01x] -> 61`, `61 0[x] -> 61` albo `550[x] -> 55` względem zakresu `[45 - 65]`, uszkodzone `[45 - 651]%` jest normalizowane do `[45 - 65]%`, a tekst zapisany w formularzu jest normalizowany bez dopisanego wymogu poziomu; błędne dopasowanie tego tekstu do `Aspekt Wewnętrznego Spokoju` jest niedozwolone,
- normalizacja OCR tarczy naprawia typowe uszkodzenia linii bazowych w warstwie importu: `[1001`, `[100]`, `[100` i `[100%` dla `+100% obrażeń od broni w głównej ręce` są prezentowane jako `[100]%`, a brakujący zakres `20,0% szansy na blok` jest uzupełniany jako `[20,0]%`; te wartości pozostają wyłącznie liniami bazowymi itemu i nie zasilają runtime DPS,
- dla affixu `Redukcja obrażeń` merger preferuje pełny, poprawnie odczytany zakres procentowy z wariantu OCR, np. `11,0 - 15,0`, nad uszkodzoną sklejką typu `11 - 0`; importer nie odtwarza brakującej granicy z katalogu affixów, więc jeżeli żaden wariant OCR nie zawiera poprawnego zakresu, formularz zostawia `Brak zakresu`; wartość i zakres pozostają danymi importu itemu i nie są aktywne w runtime DPS,
- finalny formularz importu tarczy jest testowany po pełnym flow `ItemImageImportService -> parser -> merger -> editable form -> renderer -> HTML`: `Treść efektu` musi zawierać znormalizowane `61%[x] [45 - 65]%` bez wymogu poziomu, zakres redukcji obrażeń musi być widoczny w wierszu affixu, a `greaterAffix` z jawnego markera OCR albo ograniczonej regresji Kościanych Łusek musi przetrwać do checkboxa `Gwiazdka`,
- tarcza Kościanych Łusek i aspekt `fortify_damage_increased` pozostają w tym etapie opisowym importem itemu; nie są podłączone do runtime DPS, nie zmieniają `weaponDamage`, nie aktywują efektu aspektu i nie wymagają zmian w `DamageEngine`,
- warianty OCR tarczy obejmują osobne cropy dla nazwy, affixów i aspektu; istniejący fixture obrazu `src/test/resources/items/tarcza.png` pozostaje przypadkiem Nestorskiej Egidy i nie jest używany jako obraz Kościanych Łusek,
...
- preprocessing OCR zawiera osobne warianty `weapon-stats-crop-x4`, `weapon-stats-crop-gray-x4-contrast` i `weapon-stats-crop-gray-x4-sharpen`, które celują w górną sekcję statystyk broni między DPS i attack speed bez usuwania dotychczasowych wariantów OCR,
- test importu Odłamka Verathiela musi przechodzić przez realny flow `ItemImageImportService -> OCR variants -> parser -> merger -> editable form -> renderer`; test parsera na ręcznie wpisanym tekście nie wystarcza jako dowód działania UI,
- snapshot realnego Windows OCR dla `verathiel-miecz.png` zawiera uszkodzone warianty linii zakresu, np. `(+1830) [1 350- 1 9781 pkt. obrażeń za trafienie`; końcowe `1` w takim kontekście jest traktowane jako OCR-owy błąd nawiasu `]`, a nie jako część wartości `1978`,
...
Import miecza i tarczy nie zmienia `DamageEngine` i nie implementuje `effectiveRank`. Po założeniu itemu current build może użyć strukturalnego `averageWeaponDamage` aktywnej broni jako istniejącego `weaponDamage` runtime; tarcza nie jest jeszcze liczbowym źródłem obrażeń, a jej pancerz, blok, odporności, redukcja obrażeń, aspekt opisowy, efekt unikatowy, Lucky Hit i opisowe affixy pozostają nieaktywne w DPS.
...
- pełniejszy odczyt itemu jest prezentowany osobno od sekcji `Mapowanie do aktualnego modelu buildu` jako pełny zapis itemu z nagłówkiem, implicitami, dodatkowymi liniami i socketem / gniazdem; affixy i efekt specjalny są edytowane w dedykowanych polach formularza importu,
...
Import itemu rozpoznaje potwierdzone hartowane affixy z katalogu hartowania i zapisuje je w sekcji `Hartowanie`, a nie w zwykłych affixach ani w tekście aspektu. Dla zahartowanej tarczy obsługiwany jest odczyt `Defensywa / +5 do maksymalnej liczby kumulacji Animuszu / Greater Affix`, także gdy OCR sklei tę linię z aspektem `Gdy masz umocnienie...`; parser rozdziela wtedy hartowanie od treści efektu. Sam import nie zmienia runtime; efekt `max Animusz` działa dopiero wtedy, gdy item z tym zapisanym hartowaniem jest aktywnie założony.

Import zahartowanej tarczy normalizuje potwierdzony błąd OCR nazwy `Miażdżąca Tarcza Kościanych Łusek`, w tym warianty `L U Sek`, `LUSEK` i końcowe markery gwiazdek. Rozpoznane hartowanie jest usuwane z prezentacji `Dodatkowe / sezonowe linie`, UI importu respektuje limit `1/1` i ukrywa aktywny formularz dodawania, a normalizacja aspektu usuwa znane śmieci OCR typu `Przewiń w dół` z zakresu rolla.

Import itemu z wielu screenów jest przeznaczony dla przewijanego tooltipa, który nie mieści się na jednym ekranie. Formularz pozwala wybrać `1..5` obrazów tego samego itemu; screeny trzeba dodać w kolejności od góry tooltipa do dołu. Importer wykonuje OCR każdego obrazu osobno, scala komplementarne warianty OCR z danego screena, potem scala screeny w kolejności uploadu, usuwa overlap oraz linie UI-only typu `Przewiń w dół`, `Przewiń do góry`, akcje ekwipunku, sprzedaż, trwałość i porównawcze `(Wytrzymałość: +4,6%)`. Parser dostaje kanoniczny opis itemu razem z typowanymi metadanymi linii, jeżeli merge je posiada; surowe multi-OCR variants i canonical source nie są pokazywane w normalnym UI importu. Zduplikowane bazowe/implicit linie z overlapu albo wariantów OCR, np. `20,0% szansy na blok [20,010]%` i `+100% obrażeń od broni w głównej ręce [100]%`, są deduplikowane do kanonicznych linii bazowych; `+100% obrażeń od broni w głównej ręce` jest base line, a nie zwykłym affixem. Aspekt `Gdy masz umocnienie... 61%[x]` musi przetrwać canonical merge także przy uszkodzonych wariantach OCR typu `o61`, `610[x]` i `651%`. Typowane `socket-stat:*` z per-screen merge muszą przetrwać do multi-final merge jako dane `SOCKET_GEM_RUNE`, więc finalny parser nie może zobaczyć ich wyłącznie jako gołego tekstu i utworzyć z nich ordinary affixu.
...
UI importu i edycji pokazuje sekcję `Gniazda` z wyborem `Liczba gniazd` (`0`, `1`, `2`) oraz wierszami `Gniazdo 1` i `Gniazdo 2`. Po wybraniu gema formularz pokazuje jego efekt dla kontekstu itemu; dla tarczy `Wspaniały Diament` pokazuje `+30 pkt. do wszystkich współczynników`. Dla `DETECTED_STAT` formularz nie pokazuje pustej widocznej kolumny `Gem`, tylko osobny czytelny wariant wiersza/karty z tekstem `Wykryty stat gema/runy: ...` oraz `Runtime nieaktywny`; select `socketContent_X` pozostaje dostępny, a hidden fields `socketDetected*` nadal przechodzą przez submit. Biblioteka i current build prezentują sockety jako osobne linie, np. `Gniazda · Liczba gniazd: 1 · 1: Wspaniały Diament · +30 pkt. do wszystkich współczynników · Runtime nieaktywny`. Import OCR może rozpoznać dokładną linię `Puste gniazdo` jako jedno puste gniazdo oraz wykryte staty zajętych gniazd, np. `+150 siły` i `+120 siły`, jako dwa `DETECTED_STAT`. Takie staty są widoczne w UI i przechodzą przez import form, hidden fields, submit, repository, edit form i ponowny zapis, ale nie aktywują runtime gemów/run.

Normalny ekran importu oraz ekran edycji pokazują socket/gem/rune stats wyłącznie w sekcji `Gniazda`, np. `Liczba gniazd: 2`, `Gniazdo 1`, `Wykryty stat gema/runy: +150 siły`, `Gniazdo 2`, `Wykryty stat gema/runy: +120 siły` i `Runtime nieaktywny`. Te same linie nie mogą pojawić się drugi raz jako techniczny raw dump `Pełny zapis itemu` / `Socket / gniazdo`. Na ekranie edycji `Zapisz zmiany` jest osobną akcją formularza renderowaną po zamknięciu fieldsetu `Gniazda`; przycisk nie jest częścią `socketing-fieldset`, `socketing-row` ani `socketing-detected-row`, a zmiana layoutu nie zmienia modelu `ItemSocketing`, mappera formularza, repozytorium ani `CurrentBuildRuntimeInputResolver`. Jeżeli trzeba diagnozować pełny odczyt, source raw line, typed merge metadata albo decyzje parsera, źródłem są debug logi włączane przez `-Ddstats.itemImport.debug=true`, domyślnie zapisywane do `logs/item-import-debug-yyyy-MM-dd-HH-mm-ss.log`, chyba że użytkownik poda `-Ddstats.itemImport.debug.file=...`; normalny UI nie dodaje panelu debug.
...
- `qualityCurrent=25`, `perfectedAffix=null` oznacza item 25/25 bez wskazanego aktualnego doskonalonego afiksu tylko wtedy, gdy importer nie wykrył żadnych markerów wskazujących na możliwy wybór. Jeżeli OCR zawiera lokalne markery GA / doskonalenia, ale nie da się jednoznacznie ustalić wybranego affixu, UI importu i edycji pokazuje `Wymaga potwierdzenia`, a debug zapisuje wykrytych kandydatów zamiast udawać `Brak / nie wybrano`,
...
Kontrakt importu udoskonalonych itemów: wartości widoczne w grze na itemie udoskonalonym są wartościami finalnymi po Doskonaleniu. Importer nie może zapisać ich naiwnie jako source/base `0/25`, bo spowoduje to podwójne skalowanie. Import udoskonalonych itemów rozpoznaje `qualityCurrent` / `perfectedAffix` tam, gdzie OCR daje stabilny sygnał, i może uzupełnić `referenceValue` przez `ImportedItemDisplayedValueReverseResolver` wyłącznie wtedy, gdy wynik jest jednoznaczny. Jeżeli reverse-resolve jest niejednoznaczny albo typ affixu nie ma obsługi, `referenceValue` pozostaje puste.
...
Receptura `Przeistoczenie przedmiotu / Transfigure Item` wymaga `1x` przedmiotu legendarnego, unikatowego albo mitycznego oraz `1x` `Niestabilny prastary pył / Volatile Primordial Dust`; `Pryzmat dostrojenia / Tuning Prism` jest opcjonalny. Normalny UI Przeistoczenia zapisuje stan gotowego itemu, a nie historię craftingu ani symulację losowania. Pryzmat jest częścią receptury przed wykonaniem losowania, więc nie jest pokazywany w normalnym formularzu gotowego itemu; model może nadal przechowywać `tuningPrism` dla kompatybilności, przyszłego OCR albo trybu zaawansowanego. Obsługiwane wartości pryzmatu mają polskie etykiety w UI technicznym: `Brak`, `Entropiczny`, `Kulleana`, `Agresywny`, `Pragmatyczny`, `Protektora`, `Zasobny`, `Adeptowski`, `Chromatyczny`; enumy i internal ids mogą pozostać angielskie.

Model itemu przechowuje `transfigured`, `lockedAfterTransfiguration`, `tuningPrism`, `outcome`, referencje do affixów przy upgrade/replacement, affix bonusowy lub zastępujący z wartością widoczną na itemie, `bonusQuality`, `indestructible` i notatkę. Stare itemy bez tych pól wczytują się jako `transfigured=false`, `lockedAfterTransfiguration=false`, `outcome=null/Brak`. Dla `transfigured=true` domyślny status wewnętrzny to `lockedAfterTransfiguration=true`, a `tuningPrism` domyślnie jest `NONE`; dla `transfigured=false` status blokady jest `false`, pryzmat `NONE`, a pola dynamiczne są puste. `lockedAfterTransfiguration` może zostać zachowany wewnętrznie/OCR, ale normalny UI go nie edytuje. Status `niemodyfikowalny` z gry nie blokuje technicznej edycji w aplikacji; użytkownik nadal może ręcznie poprawić importowane dane.

Affix Przeistoczenia nie wymaga od użytkownika wpisywania source rolla `0/25`. Formularz używa pola `Wartość widoczna na itemie`, bo użytkownik zwykle przepisuje wartość z gry po Doskonaleniu. UI opisuje pochodzenie wartości jako `Wartość widoczna w grze`, `Wartość bazowa/source roll` albo `Nieznane pochodzenie`. Import rozpoznaje definicję przeistoczenia po tekście / semantyce z `TransfigurationAffixCatalog`, nie po tym, czy displayed value mieści się w katalogowym zakresie. `displayedValue` przeistoczenia musi być wyciągany lokalnie przy kotwicy tekstowej definicji, np. przy fragmencie `pkt. do wszystkich współczynników`, a nie z pierwszej albo ostatniej liczby całego sklejonego bloku OCR. Parser nie może brać wartości przeistoczenia z poprzedniego ordinary affixu, bracketu poprzedniego affixu ani wcześniejszej kotwicy. Jeżeli OCR sklei segmenty i utworzy lokalny flat token typu `4115 pkt.`, importer może odzyskać wartość z końcówki tokenu tylko wtedy, gdy token leży bezpośrednio przy kotwicy przeistoczenia i lokalne okno zawiera ślad sklejenia z poprzednim bracketem/segmentem; przykład `4115 pkt. do wszystkich współczynników` z poprzedzającym artefaktem bracketu jest odzyskiwany jako `+115 pkt. do wszystkich współczynników`. Jeżeli lokalna wartość nie jest bezpieczna do ustalenia, importer zostawia przeistoczenie jako niepełne/nieznane zamiast tworzyć błędne `displayedValue` z poprzedniego affixu. Model `TransfigurationAffixRoll` zapisuje `displayedValue`, `valueProvenance` oraz opcjonalne `sourceRangeMin/sourceRangeMax` odczytane z OCR:
...
Import itemów przeistoczonych rozpoznaje finalne wartości z gry i status niemodyfikowalności; nie wolno naiwnie traktować affixów Przeistoczenia jako zwykłych source affixów bez oznaczenia pochodzenia. Linie porównawcze gry w nawiasach, np. `(Wytrzymałość: +4,6%)`, są comparison noise z porównania z aktualnie noszonym itemem: importer je ignoruje i nie zapisuje jako affix, affix Przeistoczenia ani statystykę itemu.
...
- normalny widok biblioteki i formularz edycji zapisanego itemu nie pokazują komunikatów OCR-weryfikacyjnych z importu ani sekcji `Diagnostyka OCR`; edycja pracuje na danych zapisanych w bibliotece i nie uruchamia OCR ponownie,
...
- modal szczegółów itemu pokazuje użytkowe dane zapisane w bibliotece: `Dane podstawowe`, `Base stats`, `Linie bazowe`, `Affixy`, jeden blok `Aspekt / efekt` oraz `Socket / gniazdo`; nie eksponuje statusów runtime, OCR/source/debug ani sekcji `Diagnostyka OCR` jako głównej treści popupu,
...
- filtry domenowe działają na strukturze `SavedImportedItem`: `slot`, `selectedAspectId`, lista `affixes`, `greaterAffix`, kanoniczne `ItemImportDetails` / `displayName` oraz dopiero awaryjnie `FullItemRead`, a nie na przypadkowym płaskim OCR dumpie,
...
- GUI nie pokazuje sekcji `Pełny odczyt widocznego itemu` ani technicznego `Pełnego zapisu itemu` w normalnym flow importu; raw OCR, pełne linie odczytu i typed merge diagnostics pozostają w modelu i logach debug, nie w ekranie użytkownika,
- brak sekcji `Pełny odczyt widocznego itemu` nie zmienia pól edycyjnych typu itemu: dla broni pola `DPS broni`, min/max/średnia obrażeń za trafienie i `Ataki na sekundę` są edytowane wyłącznie w fieldsecie `Dane broni`, a dla tarczy `Pancerz` i uporządkowane linie bazowe pozostają w fieldsecie `Dane tarczy`,
- GUI pokazuje ręczny formularz zatwierdzenia z edytowalną listą affixów opartą o `ApplicationAffixRegistry`: użytkownik może zmienić typ i wartość affixu, oznaczyć `Greater Affix`, usunąć wiersz z tabeli albo dodać brakujący affix z katalogu przez akcję `Dodaj affix` bez przeładowania strony; tabela affixów nie pokazuje kolumny OCR/source, chociaż źródło może pozostać w modelu, ukrytych polach albo debug danych; layout tabeli używa przewijanego wrappera i osobnej siatki `item-affix-add-grid`, aby długie selecty, checkbox i przyciski nie nachodziły na siebie,
...
- GUI po zatwierdzeniu nie pokazuje `Pełny odczyt zapisany w bibliotece`; zapisany `FullItemRead` nadal przechodzi przez submit i repozytorium jako dane roundtrip/diagnostyczne, ale ekran potwierdzenia pokazuje tylko user-facing summary, akcje biblioteki oraz domenowe sekcje itemu,
- poprawki importu mają być zabezpieczane regresjami mechanizmów, a nie stale rosnącą listą pełnych golden fixture'ów dla każdego itemu; konkretny item ze screena albo logu może być scenariuszem manualnej akceptacji, ale automatyczna ochrona ma testować klasę błędu na syntetycznym OCR/tekście, jeżeli problem jest generyczny,
- import Odłamka Verathiela rozdziela `weaponDps = 1830`, `weaponDamageMin = 1350`, `weaponDamageMax = 1978`, `averageWeaponDamage = 1664` i `attacksPerSecond = 1.10`; nowszy wariant z tooltipem `1874 / 1390 - 2018 / 1.10` rozdziela się analogicznie do `weaponDps = 1874`, `weaponDamageMin = 1390`, `weaponDamageMax = 2018`, `averageWeaponDamage = 1704`; średnia jest liczona wyłącznie z jawnego min/max, nigdy z DPS i attack speed,
...
- zakresy obrażeń broni zachowują separatory tysięcy w OCR: `1 884` jest parsowane jako `1884`, `2 512` jako `2512`, a parser nie może zaczynać dopasowania od środkowego `884` tylko dlatego, że pierwsza cyfra jest oddzielona spacją; gdy min/max są wiarygodnie sparsowane, `averageWeaponDamage` wynika z tych wartości, np. `(1884 + 2512) / 2 = 2198`,
- finalny merge danych itemu wybiera najlepszy semantycznie kandydat zakresu broni z wariantów OCR, a nie pierwszy albo ostatni tekst połączony z linii; scoring zakresu broni premiuje strukturę zakresu z kotwicą `pkt. obrażeń za trafienie`, zachowane spaced thousands, relację `min < max`, średnią zgodną z `(min + max) / 2` oraz spójność `weaponDps ≈ averageWeaponDamage × attacksPerSecond`; jeżeli jeden wariant daje `1884-2512 / 2198 / 1.10` i jest spójny z DPS `2417`, a inny daje ucięte `884-2512 / 1698` i jest niespójny z tym samym DPS/APS, finalny formularz wybiera spójny wariant `1884-2512`; artefakty OCR typu `II 884 - 2 512` są odrzucane albo karane przez niepoprawną relację min/max lub niespójność DPS/APS,
- jakość Doskonalenia pochodzi z linii jakości, np. `25 (+25) jakości`, `25 (+25) JAKOŚCI` albo `25 ( +25 ) jakości`; parser nie wyprowadza jakości z mocy przedmiotu, wartości affixów ani wartości Przeistoczenia,
- ekstraktor ordinary affixów wiąże wartość z lokalnym oknem przy dopasowanej kotwicy katalogowej, a nie z pierwszą liczbą całej sklejonej linii OCR; w jednej linii typu `+270 siły +948 pkt. zdrowia przy trafieniu Mnożnik x15% wszystkich obrażeń` każda kotwica dostaje własną najbliższą wartość, a czysty lokalny segment wygrywa z długą sklejką OCR zawierającą cudze wartości,
- ekstraktor ordinary affixów używa też kompatybilności kształtu wartości: affix mnożnikowy musi mieć lokalny wzorzec mnożnika/procentu, np. `Mnożnik`, `x` przy liczbie albo `%`; fraza `wszystkich obrażeń` bez takiego markera, np. `+948 wszystkich obrażeń`, nie wystarcza do utworzenia `ALL_DAMAGE_MULTIPLIER`,
- pełny bracketowy zakres rolla nadal jest preferowany nad wariantem bez zakresu dla tego samego affixu i wartości, ale tylko wtedy, gdy zakres został realnie odczytany albo bezpiecznie znormalizowany z tego samego lokalnego segmentu; scoring nie może tworzyć brakujących zakresów z katalogu,
- katalog affixów zawiera dla Odłamka Verathiela opisowe wpisy potwierdzone screenami: starszy wariant `Obrażenia od broni` `94 [94 - 157]`, `Maksymalne zdrowie` `2141 [1831 - 2200]`, `Zdrowie przy trafieniu` `545 [526 - 632]` i `Szczęśliwy traf: zasób podstawowy` `3 [3 - 4]`, a także nowszy wariant importu jednoręcznego miecza z affixami `+134 obrażeń od broni [94 - 157]`, `+172 siły [150 - 180]`, `+300 zdrowia za zabicie [+300]` oraz `Mnożnik x16% obrażeń z upływem czasu [15 - 30]%`; ręczna weryfikacja nowszego wariantu musi renderować dokładnie te 4 ordinary affixy z zakresami `94 - 157`, `150 - 180`, stałym `300 - 300` i `15 - 30`, bez przenoszenia ich do aspektu / efektu unikatowego,
- katalog affixów rozróżnia `Mnożnik x...% wszystkich obrażeń` od `Mnożnik x...% obrażeń z upływem czasu`; dopasowanie mnożnika wszystkich obrażeń wymaga lokalnego kontekstu `wszystkich obrażeń` albo `all damage`, żeby alias `Mnożnik x` nie przejmował linii damage over time,
- `Mnożnik x...% wszystkich obrażeń` jako ordinary `ALL_DAMAGE_MULTIPLIER` nie jest tym samym co `Mnożnik x...% obrażeń z upływem czasu` i nie jest tym samym co wynik Przeistoczenia `Mnożnik x...% obrażeń (Fizyczne)`; ordinary all-damage multiplier i fizyczny mnożnik Przeistoczenia pozostają runtime inactive/data-only, dopóki nie ma osobnego zweryfikowanego kontraktu DPS,
- Przeistoczenie może rozpoznać `Mnożnik x...% obrażeń (Fizyczne)` jako bonusowy affix Przeistoczenia `PHYSICAL_DAMAGE_MULTIPLIER` tylko wtedy, gdy nie ma silniejszego wyniku Przeistoczenia z tej samej sekcji. Gdy item ma `Przeistoczony / Niezniszczalność`, fizyczny mnożnik z dolnego/socket/gem/rune regionu pozostaje `SocketGemRuneStat` data-only, a nie finalnym wynikiem Przeistoczenia,
- manualny scenariusz akceptacji dla aktualnego realnego przypadku Odłamka Verathiela z danymi broni `2417 / 1884 - 2512 / 1.10` oczekuje `averageWeaponDamage = 2198`, jakości `25/25`, ordinary affixów w visual order: `+314 obrażeń od broni`, `+270 siły`, `+948 pkt. zdrowia przy trafieniu`, `Mnożnik x15% wszystkich obrażeń`; `+7,5% szansy na trafienie krytyczne` trafia do Hartowania jako `Ofensywa / +7,5% szansy na trafienie krytyczne` z finalnym sourceLine tej właśnie linii, `Niezniszczalność` jest wynikiem Przeistoczenia, a `Mnożnik x32% obrażeń (Fizyczne)` jest jednym statem socket/gem/rune `DATA_ONLY`. Nie wolno przyjmować wartości z cudzych kotwic tej samej sklejonej linii, np. `270` jako `Zdrowie przy trafieniu`, `14` jako `Obrażenia od broni`, `948` jako `ALL_DAMAGE_MULTIPLIER`, `47,5` jako finalny source hartowania albo `170` jako fizyczny socket multiplier, i nie wolno tworzyć z socket/gem/rune stats ordinary affixów,
- ordinary order tego scenariusza może wymagać rozdzielenia `selectedValueCandidate` od `visualAnchorCandidate`: czysty kandydat tekstowy `+314 obrażeń od broni` może pochodzić z późniejszego wariantu OCR, ale wcześniejszy zaszumiony, semantycznie zgodny kandydat z lokalną pozycją w tooltipie pozostaje kotwicą kolejności i markerów GA,
- syntetyczna regresja mechanizmu importu pokrywa wariant bez dedykowanej nazwy/aspektu: `2 417 pkt. obrażeń na sek.`, poprawne `[1 884 - 2 512] pkt. obrażeń za trafienie`, uszkodzone `[884 - 2 512]`, artefakt `II 884 - 2 512`, `1,10 ataku na sekundę`, `25 (+25) jakości`, sklejone `+270 siły +948 pkt. zdrowia przy trafieniu Mnożnik x15% wszystkich obrażeń`, czyste `+314 obrażeń od broni`, uszkodzone `14 obrażeń od broni...`, lore z zerem i słowami o umiejętnościach, które nie może stworzyć `CORE_SKILL_RANKS=0`, `Przeistoczony / Niezniszczalność` oraz `Mnożnik x32% obrażeń (Fizyczne)` jako socket/gem/rune data-only,
- stare dedykowane testy konkretnego itemu utrwalające `weaponDamageMin=884` nie są kontraktem projektu. Mechanizm spaced thousands i merge zakresu broni musi być pokryty syntetycznymi/regresyjnymi testami, które porównują wariant ucięty `884-2512` z poprawnym `1884-2512` i finalnie oczekują `1884`,
- aktualny kontrakt importu Odłamka Verathiela wymaga testu końcowego HTML formularza ręcznego potwierdzenia, a nie tylko testu parsera: HTML musi zawierać inputy `weaponDamageMin` z wartością `1350`, `weaponDamageMax` z wartością `1978`, `averageWeaponDamage` z wartością `1664`, `weaponDps` z wartością `1830` oraz attack speed `1.10`,
- affix `Zdrowie przy trafieniu` dla Odłamka Verathiela musi zachować zakres `526 - 632`, jeżeli taki bracketowy zakres został faktycznie odczytany albo bezpiecznie znormalizowany z bracketowego fragmentu OCR; importer nie odtwarza brakującego zakresu z katalogowego kontekstu itemu ani z pojedynczej granicy,
...
- affixy Odłamka Verathiela są deduplikowane po dopasowaniu katalogowym, wartości i fladze `Greater Affix`; gdy warianty OCR różnią się jakością zakresu, wpis z rzeczywistym poprawnym zakresem OCR wygrywa z wpisem bez zakresu albo z uszkodzonym zakresem, ale katalog nie tworzy brakujących `rollRangeMin` / `rollRangeMax`,
- affix `Obrażenia od broni` z Odłamka Verathiela pozostaje ordinary affixem niezależnym od `averageWeaponDamage` i nie jest sumowany z danymi broni; ten etap nie dodaje nowej matematyki runtime dla affixów Verathiela, a aspekt unikatowy pozostaje w `AspectRegistry` jako `UNIQUE`, nie jako affix,
- import Dziedzica Zatracenia jest regresją dla dwóch screenów `src/test/resources/items/dziedzic-zatracenia-helm-1.png` i `src/test/resources/items/dziedzic-zatracenia-helm-2.png`; tekst OCR może rozbijać tooltip na górną i dolną część, ale finalny item ma nazwę `Dziedzic Zatracenia`, slot `HELMET`, typ `Hełm`, rarity `UNIQUE`, flagę `Mityczny unikat`, moc `900`, pancerz bazowy `2004` i jakość `25/25`,
- wybór nazwy itemu preferuje nagłówek tooltipu nad tekstem efektu/aspektu: jeżeli jedna linia OCR zawiera nazwę oraz typ itemu, np. `Nazwa Itemu * Starożytny mityczny unikatowy hełm Moc przedmiotu: 900`, nazwa jest brana z części przed markerem/type line, a długie zdania efektu z czasownikami, warunkami albo procentami `[x]` nie mogą wygrać jako `itemName`, jeśli istnieje taki kandydat nagłówkowy,
...
- wartości `value` i `referenceValue` dla affixów rang umiejętności, w tym `CORE_SKILL_RANKS`, muszą pochodzić z OCR tej samej logicznej linii; katalog affixów nie może tworzyć ani podbijać liczby rang i nie może zmienić `+2 ... [2]` w `+3 ... [3]`,
- ordinary affix rang umiejętności wymaga jawnej lokalnej struktury dodatniej wartości, np. `+N do umiejętności: Główne` albo `+N do rang ...`; `value=0`, liczba z lore/opisu/footeru albo zdanie efektu po Przeistoczeniu nie może tworzyć `CORE_SKILL_RANKS`, nawet jeśli zawiera słowa `umiejętności` i `główne`; regiony lore/footer/description/transfiguration nie mogą po finalnym merge wracać jako ordinary affix bez jawnej struktury affixu,
- single-value bracket przy rangach umiejętności jest traktowany konserwatywnie: zamknięty bracket typu `[2]` daje `referenceValue=2`, ale uszkodzony bracket typu `[31`, `]2` albo inny niejednoznaczny fragment nie tworzy pewnego `referenceValue=3`; displayed value odczytana z lewej strony affixu może zostać zachowana, a brak pewnego bracketu zostawia `referenceValue=null`, chyba że ogólny reverse-resolve Doskonalenia daje jednoznaczną wartość bazową,
- brak bracketowego zakresu w OCR nadal oznacza brak zakresu i katalog affixów nie tworzy brakujących `rollRangeMin` / `rollRangeMax`; bracketowa wartość referencyjna mityka pochodzi z OCR, a przy uszkodzonym lub nieobecnym bracketcie może zostać uzupełniona tylko przez jednoznaczny reverse-resolve,
- dla `mythicUnique=true` wartości ordinary affixów odczytane z tooltipa są `displayed/current values`, czyli wartościami aktualnymi itemu; `referenceValue`, jeżeli istnieje, jest tylko metadanym bazowym z bracketu. Brak `referenceValue` nie oznacza, że wolno traktować `value` jako bazę do ponownego Doskonalenia,
- OCR Dziedzica Zatracenia ma regresję na uszkodzone warianty typu `+15,0% ... 112,01%`, `[12,01%`, `[201%`, `[31` oraz `szansy szczęsnwy traf`; importer normalizuje tylko takie bracketowe fragmenty, które rzeczywiście wystąpiły w OCR i są bezpieczne dla danego typu affixu, preferuje kandydat z `referenceValue` dla tego samego affixu i odrzuca przypadkowe sąsiednie single bracket, jeśli nie jest zgodne z widoczną wartością affixu. Dla `CORE_SKILL_RANKS` fragment `[31` jest niepewnym bracketem i sam nie daje `referenceValue=3`; przy `qualityCurrent=25` displayed `3` może jednak dostać `referenceValue=2`, jeśli ogólny resolver Doskonalenia potwierdzi to jednoznacznie,
...
- merge tekstu efektu/aspektu wybiera wariant zachowujący więcej semantycznych tokenów liczbowych i bracketowych, w szczególności liczby, procenty, `[x]`, nawiasy kwadratowe i warunki liczbowe; krótszy albo językowo czystszy wariant nie może wygrać, jeżeli traci mnożnik widoczny w innym wariancie OCR tego samego efektu,
...
- zapisane w repo tekstowe fixture'y `heirOfPerditionTopText` i `heirOfPerditionBottomText` reprezentują egzemplarz Dziedzica Zatracenia z zamkniętym OCR bracketem `Rangi umiejętności: Główne = 3`, reference `3`; osobny wariant regresyjny aktualnego screena reprezentuje egzemplarz z `Rangi umiejętności: Główne = 2`, reference `2`; wariant damaged real OCR reprezentuje displayed `3` bez wiarygodnego bracketu i dostaje `referenceValue=2` przez ogólny reverse-resolve przy `25/25`. Import nie wymusza żadnej z tych liczb po nazwie itemu i zachowuje liczbę wynikającą z konkretnego inputu OCR albo jednoznacznego reverse-resolve,
- zwykłe affixy Dziedzica Zatracenia importują się jako: `Szansa na trafienie krytyczne = 15,0`, reference `12,0`, bez roll range i bez Greater Affix; `Szansa na szczęśliwy traf = 25,0`, reference `20,0`, bez roll range i bez Greater Affix; `Szybkość ruchu = 25,0`, reference `20`, bez roll range i bez Greater Affix; `Rangi umiejętności: Główne` importują się z wartości OCR konkretnego egzemplarza, np. `2/referenceValue=2`, `3/referenceValue=3` przy wiarygodnym bracketcie albo `3/referenceValue=2` przy jednoznacznym reverse-resolve `25/25`, bez roll range i bez Greater Affix,
- UI importu i edycji mitycznego unikatu pokazuje w kolumnie zakresu `Brak zakresu` oraz pomocniczą `Wartość bazowa` tylko wtedy, gdy `referenceValue` faktycznie istnieje; nie pokazuje sztucznego zakresu `12 - 12`, `20 - 20` albo `3 - 3`, a affix mityka `value=25/referenceValue=null` pozostaje w preview wartością `25`, bez podglądu `31,3`,
- `+N do umiejętności: Główne` jest katalogowane i prezentowane jako opisowy ordinary affix `CORE_SKILL_RANKS`; `N` pochodzi z OCR, nie z katalogu, a dopóki repo nie ma zweryfikowanego runtime rang z itemów, nie wpływa na `effectiveRank` ani DPS,
- linia przeistoczenia `+115 pkt. do wszystkich współczynników [75 - 100]` trafia do `ItemTransfiguration` jako `ALL_STATS` z wartością prezentowaną `115` oraz source range `75 - 100`; pozostaje to warstwą przeistoczenia, a nie losowym ordinary affixem, i nie aktywuje nowego runtime bez osobnego kontraktu,
- hartowanie `+12 do maksymalnej liczby kumulacji Animuszu` jest rozpoznawane jako `Defensywa / defense_max_animus` z doskonalonym Greater Affix źródłowo `+5` oraz jakością `25/25`, więc finalna prezentacja nie wraca do zwykłego `+7`; ta sama ścieżka pozostaje ogólna dla max Animuszu i nie jest hardcodem pod hełm,
- runtime i prezentacja resolved values dla ordinary affixów `mythicUnique=true` używają potwierdzonej wartości widocznej na tooltipie i nie doskonalą jej drugi raz niezależnie od tego, czy `referenceValue` istnieje; `CRITICAL_STRIKE_CHANCE=15/referenceValue=12` pozostaje `15`, a `LUCKY_HIT_CHANCE=25/referenceValue=null` pozostaje `25`, bez przeliczeń do `18,8` albo `31,3`. Dla Paladyna poziomu `70` kryt z Dziedzica daje `5,2 + 15,0 = 20,2`, a nie `23,2`,
- efekt Dziedzica Zatracenia / Łaski Matki jest znanym aspektem unikatowym `heir_of_perdition` dla slotu `HELMET` z runtime `DESCRIPTIVE_ONLY`: tekst `Poddaj się nienawiści... 80%[x] ... Łaski Matki` jest zachowany do ręcznej weryfikacji, ale mnożnik `80%[x]` nie zmienia DPS bez osobnego kontraktu warunków działania,
...
- importer wykrywa liczbę gwiazdek Greater Affix w nagłówku itemu jako osobne metadata importu i walidacji `headerGaCount`; OCR może widzieć ikony gwiazdek jako literalne `*`, `★`, `✦`, `✧`, `⭐` albo jako bezpieczne nagłówkowe sekwencje zero-like, np. `000`, `0 0 0`, `OOOO`, `ooo`, `○○○`, `◦◦◦`,
- zero-like run może być interpretowany jako gwiazdki wyłącznie w bezpiecznym oknie nagłówka przed anchorami statów, jakości, obrażeń, affixów, aspektów, socketów, lore albo vendor/requirement; `NA WYPOSAŻENIU` / `EQUIPPED` jest statusem UI i nie kończy nagłówka ani nie jest traktowane jako lore,
- liczba gwiazdek z zero-like run wynika z liczby znaków zero-podobnych w sekwencji, a nie ze specjalnego przypadku `000 => 3`: `0 => 1`, `00 => 2`, `000 => 3`, `0000 => 4`; kombinacja `* * 000` w nagłówku daje `headerGaCount=5`,
- `headerGaCount` służy do weryfikacji zgodności z liczbą affixów finalnie oznaczonych jako `greaterAffix`; runtime nie korzysta bezpośrednio z liczby gwiazdek w nagłówku i nadal używa wyłącznie flag `greaterAffix=true/false` na konkretnych ordinary albo tempering affixach,
- niespójność `headerGaCount` i `assignedGaAffixCount` oznacza import wymagający korekty albo potwierdzenia, np. `NOT_ENOUGH_AFFIXES_FOR_HEADER_COUNT`, `REQUIRES_USER_CONFIRMATION` albo `HEADER_COUNT_MISMATCH`; w tym kroku importer nie naprawia jeszcze pełnego przypisywania GA do konkretnych affixów,
- w tym kroku nie dodaje się sztucznych testów dla gwiazdek Greater Affix, bo repo nie ma jeszcze realnych fixture itemów z gwiazdkami; walidacja nowego mechanizmu odbywa się przez debug trace na realnym przypadku OCR, np. wariancie nagłówka `ODŁAmEK VERATHIEL * * 000 Starożytny unikatowy miecz`,
...
- logger diagnostyczny używa kategorii `krys.itemimport.debug` i prefixu `[ITEM_IMPORT_DEBUG]`; jeżeli nie zmieniono ścieżki parametrem `-Ddstats.itemImport.debug.file=...`, te same wpisy są dopisywane do timestampowanego pliku `logs/item-import-debug-yyyy-MM-dd-HH-mm-ss.log`, np. `logs/item-import-debug-2026-06-02-12-45-30.log`; timestamp jest bezpieczny dla Windows,
- jawnie podany parametr `-Ddstats.itemImport.debug.file=...` jest respektowany dokładnie, bez dopisywania timestampu i bez wymuszania katalogu `logs/`,
- katalog `logs/` jest lokalnym katalogiem diagnostycznym ignorowanym przez Git; plików takich jak `logs/item-import-debug-2026-06-02-12-45-30.log` ani jawnie wskazanych lokalnych logów diagnostycznych nie należy commitować,
...
- logi techniczne obejmują sekcje `IMPORT_REQUEST`, `FILE`, `OCR_RAW_VARIANTS`, `HEADER_GA_STAR_CANDIDATE`, `HEADER_GA_STAR_REJECTED`, `HEADER_GA_STAR_SELECTED`, `OCR_NORMALIZED_LINES`, `OCR_SEGMENTER_INPUT`, `OCR_SEGMENTER_MATCH`, `OCR_SEGMENTER_OUTPUT`, `SCREEN_MERGER_INPUT`, `SCREEN_MERGER_OUTPUT`, `MERGE_DECISION`, `MERGE_REJECTED`, `MERGE_NUMERIC_TOKENS`, `ITEM_DETAILS`, `WEAPON_DAMAGE_RANGE_MERGE`, `AFFIX_CANDIDATE`, `AFFIX_REJECTED`, `AFFIX_MERGE`, `SOCKET_GEM_RUNE_CANDIDATE`, `SOCKET_REGION_LINE`, `TRANSFIGURATION_CANDIDATE`, `ASPECT_MATCH`, `GA_HEADER_VERIFICATION`, `FINAL_IMPORT_FORM`, `FORM_SUBMIT_MAPPING`, `ITEM_LIBRARY_SAVE_READ` i `RUNTIME_CONTRIBUTION`,
- dla importu pojedynczego screena `SCREEN_MERGER_OUTPUT` musi zawierać `scope=single-typed-merge`; pojawienie się starego `single-no-merge` oznacza regresję kontraktu, bo finalny `FullItemRead` nie może pomijać typowanego merge wariantów OCR,
...
- sekcje segmentera pokazują oryginalny blok OCR, dopasowaną kotwicę katalogową, kategorię `affix` / `tempering` / `transfiguration` / `aspect`, `definitionId` i dodany segment kandydacki; log segmentera jest aktywny tylko w trybie `-Ddstats.itemImport.debug=true`,
...
- `ITEM_DETAILS` dla broni loguje surowe tokeny zakresu obrażeń, znormalizowany min/max, sparsowane `weaponDamageMin` / `weaponDamageMax`, źródło średniej obrażeń oraz linię źródłową wykrytej jakości Doskonalenia, jeżeli debug jest aktywny,
- `WEAPON_DAMAGE_RANGE_MERGE` zapisuje kandydatów zakresu broni po wariantach OCR, `damageRangeScore`, `expectedDps`, wynik spójności DPS/APS, powód niższego score albo odrzucenia oraz finalnie wybrany zakres; log musi pokazać, dlaczego `884-2512` przegrywa ze spójnym `1884-2512`, jeżeli oba kandydaty są dostępne,
- `AFFIX_CANDIDATE`, `AFFIX_REJECTED`, `AFFIX_MERGE`, `AFFIX_VISUAL_ANCHOR` i `AFFIX_VISUAL_ORDER` zapisują typ affixu, wartość, `referenceValue`, `rollRangeMin`, `rollRangeMax`, `greaterAffix`, source line, klucz deduplikacji, punktację albo powód wyboru, jeżeli dana warstwa je posiada; gdy lepszy kandydat wartości zastępuje wcześniejszy wpis, log pokazuje `selectedValueSource`, `visualAnchorSource`, `rejectedVisualOrder`, `selectedVisualOrder`, `visualSourceOrder` i powód propagacji markera GA, a finalny `AFFIX_VISUAL_ORDER` pokazuje kolejność po deduplikacji używaną przez limit ordinary affixów; dla mnożników logują też oczekiwany i wykryty kształt wartości, a kandydat `ALL_DAMAGE_MULTIPLIER` bez lokalnego markera mnożnikowego jest odrzucany z powodem `missing multiplier marker` / `incompatible value shape`; dla `CORE_SKILL_RANKS` log odrzuca lore/footer/opis z powodem braku jawnej lokalnej kotwicy ranków albo niedodatniej wartości,
- `MERGE_SEGMENT_DECISION` zapisuje dla zaakceptowanych segmentów mergera `parentLineRegion`, `segmentStart`, `segmentEnd`, `firstAspectEffectStart`, `firstSocketGemRuneStart`, `firstLoreVendorRequirementStart`, `effectiveSegmentRegion`, `localAnchorType` i decyzję `acceptedAsOrdinary` / `acceptedAsTransfiguration` / `acceptedAsTempering` / `acceptedAsAspect` / `acceptedAsSocket` / `acceptedAsSocketGemRuneData`,
- `MERGE_TYPED_LINE`, `MERGE_TYPED_OUTPUT`, `SOCKET_STAT_PRESERVED_ACROSS_MERGE`, `SOCKET_STAT_DEDUP_KEY` i `SOCKET_STAT_OCCURRENCE` pokazują, że metadata `socket-stat:*` przetrwała per-screen merge, multi-final merge i nie została przetworzona ponownie jako `affix:*`; klucz occurrence musi zawierać offsety segmentu albo równoważny stabilny identyfikator wystąpienia,
- `SOCKET_GEM_RUNE_CANDIDATE` / `SOCKET_REGION_LINE` zapisują linie regionu gniazd/gemów/run z powodem wskazującym region socket / gem / rune, flagą `ignoredForOrdinaryAffixes=true`, `parentLineRegion`, `segmentStart`, `segmentEnd`, `firstAspectEffectStart`, `firstSocketGemRuneStart`, `firstLoreVendorRequirementStart`, `effectiveSegmentRegion`, `localAnchorType`, `decision=ignoredAsSocketGemRune`, `derivedFromSplit=true` dla segmentów wyciętych przez merger oraz `matchedAffixType`, jeśli ta sama linia technicznie pasuje do `AffixRegistry`; dla fizycznego mnożnika socket/gem/rune log musi wskazywać lokalne wiązanie wartości z `Mnożnik x...%`, żeby było jasne, że liczby unique effect / roll range zostały zignorowane,
- `LINE_CLASSIFICATION` zapisuje finalne decyzje `ordinary`, `tempering`, `socketGemRune` albo `ignored` z `selectedVisualOrder`, `visualSourceOrder`, `selectedValueSource`, `visualAnchorSource` i powodem; dla fallbacku `Mnożnik x...% obrażeń (Fizyczne)` przy `Niezniszczalność` powód ma wskazywać, że linia została zaklasyfikowana jako socket/gem/rune data mimo braku typu `SOCKET`,
- `HEADER_GA_STAR_CANDIDATE`, `HEADER_GA_STAR_REJECTED` i `HEADER_GA_STAR_SELECTED` zapisują nagłówkowych kandydatów Greater Affix po wszystkich wariantach OCR: `variantId`, `headerText`, `literalStars`, `zeroLikeRunStars`, `detectedCount`, `reliable` i źródło `OCR_HEADER_LITERAL_STARS`, `OCR_HEADER_ZERO_LIKE_RUN_HEURISTIC` albo `OCR_HEADER_MIXED`; dla wariantu `* * 000` wybrany dowód ma pokazać `HEADER_GA_STAR_SELECTED detectedCount=5`,
- `GA_MARKER_DETECTION`, `GA_MARKER_SUMMARY`, `GA_MARKER_FINAL_STATE` i `GA_HEADER_VERIFICATION` zapisują lokalne przypisanie markera GA do kandydata affixu, nagłówkowe `headerGaCount`, liczbę markerów globalnych, liczbę markerów lokalnie przypisanych, `assignedGaAffixCount`, `ordinaryAffixCount`, status walidacji, propagację z `visualAnchorCandidate` do finalnego affixu, finalny `greaterAffix` oraz `ambiguousGaMarkersRequiresConfirmation`; przy markerze globalnym albo nagłówkowym bez bezpiecznego przypisania UI ma dostać `REQUIRES_CONFIRMATION / ambiguous_ga_markers` albo ostrzegawczy status walidacji oraz per-affix stan potwierdzenia,
- `TRANSFIGURATION_CANDIDATE` zapisuje rozpoznaną definicję przeistoczenia, kotwicę/alias, lokalne okno wartości, selected displayed value, odrzucone tokeny liczbowe z powodem oraz source range, jeżeli OCR go dostarczył; dla `Mnożnik x32% obrażeń (Fizyczne)` loguje dopasowanie `PHYSICAL_DAMAGE_MULTIPLIER` jako wynik Przeistoczenia runtime inactive/data-only, a nie ordinary affix; `ASPECT_MATCH` zapisuje `selectedAspectId`, status runtime i kanoniczny tekst efektu użyty w formularzu,
- `FINAL_IMPORT_FORM` zapisuje finalne metadane itemu, ordinary affixy, `finalRenderedOrder`, hartowanie, przeistoczenie, aspekt / efekt, socketing (`emptySocketCount`, `occupiedSocketCount`, `totalSocketCount`) i pola `value`, `referenceValue`, `rollRangeMin`, `rollRangeMax`, `greaterAffix`, `greaterAffixConfirmationRequired`, `selectedValueSource`, `visualAnchorSource` oraz `visualSourceOrder` dla affixów; dla wykrytych socket/gem/rune stats loguje `sourceCategory=socketGemRune`, `displayText`, `normalizedText`, `value`, `matchedAffixType`, `sourceRegion`, `runtimeStatus=DATA_ONLY` i `ignoredForRuntime=true`,
- dla hartowania `FINAL_IMPORT_FORM` loguje osobny wpis `TEMPERING_FORM`, który rozróżnia `storedValue`, `ocrDisplayedValue` albo `importedDisplayedValue` oraz `resolvedValue`; `storedValue` jest wartością technicznie zapisaną w itemie, np. wartością katalogową/GA, a `resolvedValue` jest wartością po Doskonaleniu używaną do prezentacji UI i runtime,
- `TEMPERING_FORM` dla hartowania zapisuje co najmniej `definitionId`, `category`, `sourceLine` z finalnego selected candidate jeśli istnieje, `storedValue`, `greaterAffix`, `masterworkingQuality`, `perfectedAffix`, `resolvedValue`, `resolvedDisplayText`, `runtimeStatus` i `reason`; log nie może sugerować, że source `+5` jest finalną wartością widoczną, jeżeli UI/runtime używa resolved `+12`, ani że odrzucony artefakt `47,5` jest źródłem finalnego `7,5`,
- przykład diagnostyczny dla `defense_max_animus`: OCR/source line `+12 do maksymalnej liczby kumulacji Animuszu`, `ocrDisplayedValue=12`, `storedValue=5`, `masterworkingQuality=25/25`, `perfectedAffix=TEMPERING_AFFIX:defense_max_animus`, `resolvedValue=12`, `resolvedDisplayText=+12 do maksymalnej liczby kumulacji Animuszu`; powód powinien wskazywać, że zapisana wartość jest source/GA import value, a finalna wartość przechodzi przez resolver Doskonalenia,
...
- `RUNTIME_CONTRIBUTION` zapisuje aktywne itemy, slot, item id / nazwę, affix type, stored value, `referenceValue`, roll range, `greaterAffix`, resolved runtime value, informację czy resolver Doskonalenia zmienił wartość oraz finalne `criticalChanceFromItemsPercent` i `criticalChancePercent`, jeśli dotyczy. Wykryte socket/gem/rune stats mogą być logowane jako `RUNTIME_SOCKET_GEM_RUNE`, ale wyłącznie z `ignoredForRuntime=true` i bez wkładu do runtime statów,
- `RUNTIME_CONTRIBUTION` dla hartowania używa tego samego resolvera co runtime/UI i zapisuje `storedValue`, `resolvedValue`, `resolvedDisplayText`, `masterworkingQuality`, `perfectedAffix` oraz `reason`; dla `defense_max_animus` przy `25/25` i doskonalonym hartowaniu log ma pokazać source `5` oraz resolved `12`, a nie samą mylącą wartość `+5`,
...
- formularz edycji zapisanego itemu jest spójny z formularzem importu: ma blok danych podstawowych, fieldset `Dane broni`, jeden blok `Aspekt / efekt` z selectem i treścią efektu oraz tabelę affixów z kolumną `Zakres rolla`, bez widocznej kolumny OCR/source,
- surowy OCR w edycji nie jest blokiem danych itemu ani widoczną diagnostyką; zapis edycji używa zatwierdzonych danych z biblioteki,
...
- strukturalny podział `tarcza.png`: `1 131 pkt. pancerza` jako base stat, `45% redukcji blokowanych obrażeń`, `20,0% szansy na blok` i `+100% obrażeń od broni w głównej ręce` jako linie bazowe, a `+494 cierni`, `+7,0% szansy na szczęśliwy traf`, `13,2% redukcji czasu odnowienia` i `+114 siły` jako właściwe affixy,
- zachowanie zakresu `+114 siły [107 - 121]`, brak sklejania `+7,0% szansy na szczęśliwy traf [7,0` z `13,2% redukcji czasu odnowienia` oraz ostrożne oznaczanie `Greater Affix`: jawne markery OCR, np. `*`, `★`, `⭐`, `✦`, `•` albo podobny glif gwiazdki/punktu na początku linii, ustawiają `greaterAffix=true`, a single-value bracketowy zakres może ustawić `greaterAffix=true` tylko wtedy, gdy displayed value równa się `normalRangeMax × 1,25`; sam brak bracketowego zakresu rolla nie wystarcza do automatycznej gwiazdki,
...
- prezentację aspektu: import i formularz edycji pokazują jeden blok `Aspekt / efekt` oparty o `selectedAspectId` i opis efektu z `AspectRegistry` / `uniqueEffectText`, bez pomocniczego raw OCR effect w głównym formularzu; normalny widok biblioteki pokazuje finalny aspekt i opis z registry bez raw OCR effect,
...
- brak renderowania pełnego odczytu itemu jako normalnego podglądu importu; affixy są dostępne w ręcznej weryfikacji affixów, aspekt / efekt w pojedynczym bloku `Aspekt / efekt`, socket/gem/rune stats w sekcji `Gniazda`, a raw/pełny odczyt pozostaje wyłącznie danymi roundtrip oraz diagnostyką debug logów,
...
- rozpoznanie slotu `HELMET` dla tekstu `Starożytny mityczny unikatowy hełm` oraz regresję dwóch realnych fixture'ów Dziedzica Zatracenia przez `ItemImageImportService` ze stubem OCR opartym na tekście ze screenów,
