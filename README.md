...
- OCR oznacza `greaterAffix=true`, gdy rozpoznany edytowalny affix zaczyna się od markera `*`, `★`, `⭐` albo `✦`; jeżeli ten sam affix występuje w kilku wariantach OCR, deduplikacja zachowuje wariant z gwiazdką,
...
- aspekt tarczy z tekstu `Gdy masz umocnienie, zadajesz obrażenia zwiększone o X%[x] [45 - 65]%.` mapuje się do `fortify_damage_increased` / `Umocnienie: zwiększone obrażenia`; konkretny roll `X` pochodzi z odczytu itemu, może być naprawiony z OCR typu `610[x] -> 61` albo `550[x] -> 55` względem zakresu `[45 - 65]`, a tekst zapisany w formularzu jest normalizowany bez dopisanego wymogu poziomu; błędne dopasowanie tego tekstu do `Aspekt Wewnętrznego Spokoju` jest niedozwolone,
...
- finalny formularz importu tarczy jest testowany po pełnym flow `ItemImageImportService -> parser -> merger -> editable form -> renderer -> HTML`: `Treść efektu` musi zawierać znormalizowane `61%[x] [45 - 65]%` bez wymogu poziomu, zakres redukcji obrażeń musi być widoczny w wierszu affixu, a `greaterAffix` z jawnego markera OCR musi przetrwać do checkboxa `Gwiazdka`,
